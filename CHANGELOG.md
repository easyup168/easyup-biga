...
### 新增 · 第二个私有 skill：`serper-search`（Google 搜索 API）

同样放 `~/.openclaw-biga/local-skills/`，走 `skills.load.extraDirs`，
仓库里不出现任何内容。`skills list --agent main` 已能看到它
（`Source: openclaw-extra`）。

⚠️ `skills list` 在配了多个 agent 时**必须带 `--agent <id>`**，
否则它不知道该按谁的白名单算 —— 这不是错误，是它没法替你选。

### 🔴 坑 · `if response` 判的不是「有没有响应」，是「成不成功」

`requests.Response.__bool__` 返回的是 `.ok` ⇒ **4xx/5xx 时为 `False`**。
搬过来那份里有两处都写成 `if e.response:`，于是两处**恒假**：

| 本意 | 实际 |
|---|---|
| 取错误正文 | 永远取不到，落到 `str(e)` —— 只剩 `"400 Client Error: Bad Request for url: …"` |
| 「401/400 不重试」 | **从不生效** ⇒ 对着一个必然失败的请求重试三次 |

实测代价：真实原因是 `{"message":"Not enough credits"}`，
而看到的是一句毫无信息量的 `Bad Request`，白查了一轮。

⇒ 判 `e.response is not None`，不要判真值。
**这类 bug 不报错，它只是让你看不见原因。**

### 🔴 坑 · 错误信息把常量当成了事实

原文写死 `f"API 请求失败（{MAX_RETRIES}次重试后）"`。修好短路之后，
401/400 只试 1 次，它**照样说「3次重试后」** ——
排查的人会照着这句去找「为什么重试三次都失败」，而真相是压根没重试。

⇒ 改成报**实际发生的**次数。实测确认：1 次尝试、3.2s（纯网络往返，
无重试等待），改前是 3 次 + 2 次 `sleep`。

> 这是 R-3 的同一个形状：**一句没被验证的断言，比不写更糟。**

### 变更 · 密钥补进 profile 的 `.env`

这把 key 原本只在登录 shell 的 profile 里 `export`，而 **systemd 用户单元
不读 profile** ⇒ 网关及其子进程（skill 脚本）看不到它。
实测：网关 pid 的 `/proc/<pid>/environ` 里两个变量都是 **0** 次。

⇒ 统一放 `~/.openclaw-biga/.env`（`chmod 600`），与妙想那把一致。
**仍然需要重启网关才生效** —— 见 [0.9.11] 那条。

### 已知问题

- **Serper 账户无余额**：`{"message":"Not enough credits"}`。
  端口本身已验证连通（请求发得出去、错误正文拿得到、不重试），
  **差的是账户充值，不是代码**
- 网关尚未重启 ⇒ agent 目前还看不到这两把 key

---

## [0.9.11] - 2026-09-26

### 实测 · 三个能力都跑通了（2026-09-26）

资讯搜索（12 条真实研报）、金融数据（`600519` 收盘价 + PE）、智能选股
（命中 87 只）。金融数据那条顺带印证了交易日历：最新收盘是 **2026-09-24**
（09-25 中秋休市）。
...
| | |
|---|---|
| 额度常量写着 50，厂商给的是 **150** | 后果不是「统计不准」：客户端在 `used >= DAILY_LIMIT` 时**直接拒绝发请求** ⇒ 还剩 100 次就罢工，而错误信息说「今日额度已用完」。改成 150 并允许 `MX_DAILY_LIMIT` 覆盖 |
| 本地用量被当成厂商的余额 | 实测：厂商控制台显示今日已用 1 次，本机库显示 0 —— 那 1 次是别处发的。把本地数叫 `remaining` 等于宣称我们知道厂商余额。改成 `used_here` / `remaining_if_only_here` + 一句 `note` |

### 🔴 `.env` 改完必须重启网关

`.env` 在**进程启动时**读进 `process.env`，skill 脚本作为网关子进程继承它。
网关先起来就看不到之后新增的行 —— **而且不会报错**，只会让 skill 以
「没配密钥」的方式失败。

实测判据：网关 pid 的 `/proc/<pid>/environ` 里该变量出现 **0** 次。

⚠️ 直接跑脚本不受影响 ⇒ **「命令行能跑」不代表 agent 也能用。**
...
> eastmoney dataset，现在两个有了第二条路。

### 新增 · `sina_boards` —— 参考同机那套实例的选择

它的代码里写着「主数据源：新浪 `newFLJK.php`；备用：东财 push2，**部分环境被拒**」
—— 在一个每天跑的生产系统里，主备关系与本仓库**正好相反**。
我们 2026-09-26 撞上的正是「被拒」。

实测：行业 84 个 / 概念 175 个，各约 2 秒。

⚠️ 口径少两个字段（主力净流入、板块内涨跌家数），但**降级会被看见**：
消费方早就为此写过一条 missing（外部评审 F6 逼出来的）——

> 主力净流入 —— N/N 个板块接口未给该字段，**排名不成立**

这三个字段写 `None` 而不是 `0`：0 会让「没有资金进出」和「没给这个字段」
长得一模一样。

### 🔴 修复 · Evidence 的 `source` 会说谎

`sector-calc` 里写死 `f"em:clist/{kind}"`。板块有了备用源之后，
降级过的那天 Evidence 会指着一个**没供过数的源**。

> 那比缺字段更糟：**缺字段会进 `missing[]`，说谎不会。**

⇒ `FrozenDataset` 带上 `provider_id`；`read_boards` 返回它；
skill 用新增的 `source_prefix_of()` 从注册表拼前缀
（不能自己拼 —— `sina` / `sina_boards` / `sina_eod` 前缀都是 `sina`）。

### 字段位序是**对拍**出来的

新浪响应的第 9~11 位**不是**「价、涨跌额、涨幅」那种直觉顺序。
拿两只龙头股与新浪实时行情逐位对拍确认：`sz000829` 昨收 9.06 / 现价 9.32
⇒ 涨幅 2.870%、涨跌额 0.260，与第 9、11 位吻合。

> 猜错这个顺序不会报错，只会让领涨股的涨幅变成价格。

⚠️ 同花顺热榜被否掉了 —— 它是热门**股**，不是板块快照，口径根本不对。
我上一轮把它列成「候选」是错的。
...
> `cn.market.breadth` 配上备用源 —— 三个在真机出卡时同时失败的 eastmoney
> dataset，修好了第一个。

### 新增 · `sina_breadth` —— 主源「报数」，它「数数」

东财直接给 `f104/f105/f106`，新浪不给家数、只给每只票的涨跌幅 ⇒ 我们自己数。
实测：涨 1120 / 跌 4305 / 平 137，8~12 秒。

口径差异显式写明，**不抹平**：多了北交所（347 只）、「平盘」的定义变成我们定的。

🔴 **不去掉北交所来「对齐」主源** —— 那是硬凑：为了让两个数看起来可比，
而丢掉真实观测到的一部分市场。

⚠️ 实测过没有更便宜的替代：东财 `ulist.np` 只存在于挂掉的那组 host，
同花顺 `realhead` 502、涨跌幅排行返回 HTML，百度返回空。

### 🔴 加备胎那一刻差点让系统更脆

`ProviderChainExhausted` 的基类是 `RuntimeError`，而 `freeze_required`
只接 `(SourceError, ValueError)` 当可降级失败。于是给 breadth 配上备胎的
那一刻，「两个源都挂」从**「这条数据缺失」变成了「整张卡出不来」**。

> **降级链的终点必须仍然是一次可降级的失败。**

测试当场抓到（stub 掉整条链后炸在 `ProviderChainExhausted` 上）。
基类改成 `SourceError`。
...
第一版把 `"0.000"` 写进统一的空值表 ⇒ **平盘家数恒为 0**。
实测：首页 2000 只里 58 只的 `changepercent` 就是 `"0.000"`，
它们有成交量、有真实价格。

> 一个恒为 0 的计数不会报错，只会让人以为那天市场没有平盘。

同理停牌按**有没有价**判，不按涨跌幅 —— 停牌票的涨跌幅可能是 `0.000`，
按涨跌幅判会让停牌多的日子看起来像市场很平静。

### 变更 · 全市场分页从 12 页降到 3 页

页大小 500 → 2000。实测：`num=6000` 一次返回全部 5568 行、`num=8000` 仍是
5568 ⇒ **没有静默截断**。取 2000 而不是 6000：一次 1.6 MB 的请求失败就全丢，
三次中等请求的单次失败面更小。

> 不是「越少请求越好」，是「单次失败的代价要可控」。

日线取数因此从 28 秒降到 8 秒。

### 变更 · 新浪的行带上市场

`symbol` 形如 `sh600000`，前两位就是市场 —— 这个源知道，就不该让下游
按代码前缀去猜。猜的后果不是报错：`000001` 在沪是上证指数、在深是平安银行。

---

## [0.9.7] - 2026-09-26

> 接上通达信官网盘后包 —— 它有别人都没有的能力：**按指定交易日取**。
> 接的过程中撞出三处只有真实数据才能暴露的问题。

### 新增 · `tdx_daily_package` —— 日线的第二备用源，也是唯一能补历史的

日线链变成 `sina_eod → tdx_daily_package → eastmoney_eod`（三个风控面）。

**验证**：用 2026-09-24 的真包与新浪同日快照**逐只对拍 5556 只**，
收盘价与开盘价**零处不一致**。

⚠️ 它在非交易日返回 404 —— 那是正确行为。探针第一版用「今天」的日期，
而那天是中秋，404 被当成故障查了半天 ⇒ 普查工具改用固定的已知交易日。

### 🔴 修复一 · 同一个代码在两个市场是两只标的

盘后包按市场分文件。实测 2026-09-24 那个包：**840 个代码在两个市场都存在**，
最有名的是 `000001` —— 沪市是上证指数（3888 点）、深市是平安银行（11.3 元）。

**按代码拍平会让上证指数的点位变成平安银行的「股价」，而且不报错。**

⇒ `EodBar` 新增 `market_hint`（provider 声明的市场，给了就以它为准）。

### 🔴 修复二 · 「6 位代码 → 交易所」三份实现收口成一份

新增 `providers/instrument_segments.py`，并由 AST 扫描钉住不许复发。

收口时抓到两个**一直是错的**：

| | |
|---|---|
| `88` 被当成北交所 | 真包里 1120 条 `88xxxx` **全在 sh 文件里**，是通达信自编板块指数（`880001 总市值`）。此前没有 provider 返回过它，所以没人发现 |
| 枚举三位前缀 | 已改两位号段（`30` 覆盖 300/301/302…），交易所的分配本来就是两位粒度 |

同机另一套长期运行的实例为同一件事做过普查：同一个判断曾有二十多处实现、
大半是错的，失败形状正是**静默取到另一只真标的**。

### 🔴 修复三 · 东财 `f13` 不是无歧义的市场标识

`0` **同时**表示深市和北交所（`430047` 这类老北交所代码的 `f13` 就是 0）。
把它当权威会让整个北交所被判成「代码段与市场冲突 ⇒ 不是 A 股」，
而那不会报错，只会让 universe 悄悄少一个交易所。

⇒ 数字市场编码一律当作「provider 没说」，回退到号段规则。

> **有歧义的信息不如没有信息 —— 后者至少不会把人引向错的结论。**

### 变更 · 备用源的顺序：声明即优先级

`fallback_providers` 原来被**按 id 排序**，理由是「确定性」。
元组本身已经确定，排序买不到额外确定性，却吃掉了声明里的优先级 ——
那是「看起来能控制某件事、实际不能」的配置。

⚠️ VALIDATOR 仍按 id 排：那里顺序**没有含义**（校验源是全部都要问的）。
在没有含义的地方排序让输出稳定；在有含义的地方排序是把含义丢掉。

### 变更 · 补历史的 point-in-time 边界

universe 原来按 `retrieved_at` 解析。补 2023-01-03 时那个时刻是**今天** ⇒
2023 年的行情会配上今天的在册名单。**不会报错，只会让回测里出现一批
当时还没上市的票** —— 而幸存者偏差只会让回测好看。

⇒ 改按**交易日当日 23:59:59**。补历史因此 fail-closed（没有那天的名单快照），
那是诚实的红，与 `security_master`「只宣称从第一次成功同步那天起」同一条原则。

配套：快照型 provider 在补历史时抛 `SourceError` 而**不是从链里删掉** ——
这样会落进 `provider_attempts`，库里能查到「那天为什么没走主源」。

### 坑 · 我自己删掉了既有测试夹具

为了换一种夹具做法跑了 `rm -rf tests/fixtures`，而那个目录里**已经有**
五份别的夹具（交易日历、证据 id 向量…）。11 个 ERROR + 4 个 FAILED，
`git checkout` 恢复。

> 删之前先看目录里有什么。这次是 git 兜住的，不是判断兜住的。

---

## [0.9.6] - 2026-09-26

### 新增 · `tools/verify/source_survey.py` —— 数据源普查，可复跑

18 个候选端点逐个探活，产出一张「这台机器、此刻」的可用性表。
**总是整批跑，不支持只探一个** —— 一次失败什么都证明不了，
对照组才分得出「我被限流了」（该等）和「那一组挂了」（该走降级链）。

判据打在**形状**上，不是 HTTP 200：限流页、空壳 JSON、登录跳转都是 200。
实测抓到一个：财联社返回 200 + `{errno,msg}`。

首次结果 **12 / 18 可用**。

### 🔴 对照组读出来的两条结论

1. **东财是按 host 组挂的，不是整体限流** —— `push2`/`push2his` 全挂，
   同一时刻同一 IP 下 `push2ex`/`datacenter` 正常
2. **故障会扩散** —— `push2his` 上午 200，下午连接被关闭。
   「现在它还好」不等于「一小时后它还好」，这正是备胎要**提前**探活的理由

### 变更 · `docs/design/data-sources.md` 重写为实测版

加入普查结果、五个无备胎 dataset 的**实测可用候选**、
以及同机另一套长期运行的实例**每天真的在调**哪些端点。

普查与实战应用**不能互相替代**：前者答「能不能取到」，后者答「哪条路扛得住天天跑」。

🔴 **通达信官网盘后包**单独记一笔 —— 它有别人都没有的能力：

| | 新浪 / 东财 | 通达信盘后包 |
|---|---|---|
| 取数方式 | **快照**，只能拿「此刻」 | 按**指定交易日**下载 |
| 补历史 | ❌ | ✅ 实测取到 2023-01-03 |
| 风控面 | sina / eastmoney | **tdx.com.cn**，第三个面 |

这条能力直接影响上线验收：**某天定时器没跑成可以事后补**，
而快照端点错过就永远错过。
...
| 坑 | 后果 |
|---|---|
| 盘后包探针用「今天」 | 它在**非交易日返回 404**。第一版写了中秋当天，404 被当成故障查了半天 ⇒ 改用固定的已知交易日 |
| 只判 HTTP 200 | 见上，财联社当场抓到 |

⚠️ 顺带更正一处：2026-09-25 是**中秋休市**，最后一个交易日是 09-24。
我此前按「周五=交易日」推断过一次，错了。

---

## [0.9.5] - 2026-09-26

> 日线的主源换成新浪，东财退成备用。依据不是「东财那天挂了」，
> 是**同机另一套长期运行的实例每天走的就是它**，以及公开源目录把东财
> 标为「共用同一套风控、IP 被封会成片失联」。

### 🔴 修复 · 我自己写的一个会差 100 倍的错误

第一版 `sina_eod` 按「新浪给的是手」写了 `volume * 100`。
参考同机那套实例时看到一句注释：

> 成交股数用 `amount/close` 推，不用 `volume` —— volume 单位跨某日变过。

实测 `(amount/close)/volume` 中位数 = **1.006**（p5/p95 = 0.99/1.02）——
单位是**股**。×100 会让全市场成交量大 100 倍，**而那不会报错**，
只会让某天的换手率、量比全部失真。

⇒ 不写死倍数，改成**每次取数都自检**（窗口 0.5~2.0，抓数量级不抓精度）。
上游哪天真把单位改回「手」，比值跳到 ~100，当场 fail closed。
样本不足 20 只时宁可失败，不假装查过。

> 这道守卫是用**别人的事故**换来的 —— 那句注释背后是一次真实的单位漂移。

### 变更 · `cn.equity.daily_bars` 主备对调

| | 主源 | 备用 |
|---|---|---|
| 之前 | 东财 `clist` | 无 |
| 现在 | **新浪 `getHQNodeDataSimple`** | 东财 `clist` |

⚠️ 口径差异写在适配器模块头，不当等价替换：新浪**不自报总数**
（「declared_total vs 实收」那道交叉校验因此退化成恒等式）、
**停牌股不返回**（东财返回但 OHLC 是 `"-"`）。

🔴 停牌股不返回**不是缺陷，但它改变了下游的理由**：`tradability` 用
「在 universe 里、当日没有价」区分停牌与数据缺失，两个源在那里归到同一种
状态 —— 结论不变、理由不同，别把「没返回」读成「源坏了」。

⚙️ 实测：5568 行 / 28 秒（东财 54 页，新浪 12 页 × 500）。

### 变更 · 日线归一化层不再认识东财的字段名

新增 provider 中立的 `EodBar`，与 `SecurityListing` 同一个理由。
`provider_id` 从模块常量变成**参数** —— 降级过的那天，每一行的溯源
必须写实际供数方。

### 新增 · `docs/design/data-sources.md` —— 数据源全景

回答三个「换源时才会问、而那时已经来不及查」的问题：有没有第二条路、
口径一样吗、**换源之后还剩几道校验**。

🔴 第三条最容易漏：`cn.security_master` 的「自报总数 vs 实收条数」
交叉校验，在降级到新浪时两边恒等 —— **它还在跑，但什么都没查**。

> 一道在降级时退化成恒等式的校验，比没有这道校验更危险：
> 它让人以为那天也被查过。

同时记下两处已知待办：五个盘中 dataset **一个备胎都没有**；
「6 位代码 → 交易所」在本仓库有**两份**实现。

---

## [0.9.4] - 2026-09-26

> `cn.security_master` 第一次真的同步了：**5568 只，三个交易所齐全**。
> 而它是**经备用源**同步的 —— 主源正在真实故障。
> 顺带把 `PRIMARY_FALLBACK` 演练关掉了，用的是真故障，不是制造的。

### 🔴 诊断更正 · 不是「东财整体限流」，是 `push2*` 这一组 host 的上游故障

仓库里（探活工具、TODO、模块头）一直写着「东财会整体限流 —— 被限流时连已知
可用的端点也返回 502」。2026-09-26 实测**推翻了它**：

| 同一时刻、同一 IP、同样的请求头 | |
|---|---|
| `push2` / `push2delay` / `N.push2` | ❌ 502（nginx 默认页）|
| `push2ex` / `push2his` / `datacenter-web` / `quote` | ✅ 200 |

四条排除证据：换简单过滤串 → 还是 502；同 host 换端点 → 还是 502；
带完整浏览器头 → 还是 502；同 IP 其它三组 host 全 200。

> **一次失败什么都证明不了，对照组才让 502 变成信号。**

分不清「我被限流了」和「那一组挂了」的代价很实在：**前者该等，后者该走降级链**。
原来那句话会把人一直引向「等一等」。

### 新增 · `sina_security_master` —— 被真故障逼出来的备用源

注册表里 `cn.security_master` 的 `fallback_providers` 原来是**故意留空**的，
理由写着「登记一个没验过的备胎，等于承诺一条可能也是断的降级路径」。
那句话仍然成立 ⇒ 这次登记的这个是**真跑通过**的：5568 只、111s、三个交易所齐全。

口径差异显式写在适配器模块头里，**不当等价替换**：

| | 东财 `clist`（primary） | 新浪 `hs_a`（fallback） |
|---|---|---|
| 成员 | 按板块过滤串取在册 A 股 | 行情节点，**当前在交易的** |
| 上市日 | 有 | **没有** ⇒ `list_date=None` |
| 自报总数 | 有（交叉校验分母）| 没有 ⇒ **那道交叉校验在降级时天然失效** |

🔴 **为什么不用东财 `datacenter-web`**（它字段更齐、我上一轮推荐的就是它）：
探活推翻了。`RPT_F10_BASIC_ORGINFO` 是**机构信息表**，24780 条，含
`PT金田A` / `国华退` 这类早已退市的，而**没有退市列可筛** —— 唯一能分开的
信号是名字里的「退」「PT」字样，按字符串形状分类正是 L-13。

> **字段齐但口径错，比缺一个可空字段糟得多。**

缺上市日不影响 universe（`list_date` 可空、`security_universe_at()` 不读它），
且会体现在质量指标 `missing_list_date_count` 上 —— **看得见的缺，不是隐藏的缺**。

### 变更 · 归一化层不再认识东财的字段名

`normalize_security_master_rows` 原来直接读 `f12` / `f14` / `f13` / `f26`。
只有一个源时看不出问题，**加第二个源的那一刻就咬人**：要么让新 provider 把
字段伪装成 `f12`（读的人会以为它是东财），要么在归一化层加一个「这是哪家」的
分支（同一个判断两份实现）。两条都错 ⇒ 新增 provider 中立的 `SecurityListing`，
翻译交给各自的适配器。

### 修复 · 🔴 `302` 代码段不认识 —— 第一次真实同步本来必然整体失败

`302132 中航成飞`，成交活跃的创业板股。创业板早期只有 300/301，后来多出 302 段。
在补上它之前，**这个数据集的第一次真实同步必然 fail-closed** —— 主源走这条路
也一样，只是它从没跑到过这一步。

> 全市场 5568 只里恰好 1 只命中。这种洞只有**真实数据**能暴露。

顺带：不认识的前缀现在**攒起来一次报全**，不是撞上第一个就死。
死在第一个的代价不是「慢」：一次只暴露一个新代码段，要重跑 N 次、
每次等一分多钟才知道全貌。fail-closed 本身不放松。

### 修复 · 降级过的那天，库里不能看起来像正常的一天

两处：

1. **溯源写实际供数方**，不是 dataset 的 primary（分区、fact 行都改了）
2. `SnapshotPublishRequest` 新增 `failover_attempts` —— 把**主源失败那一半**
   也落进 `provider_attempts`

第 2 条是关键：服务原来一律记「一个 raw artifact = 一次 PRIMARY 成功」。
那对单源数据集是对的，**对降级过的那次就是假的** —— 库里只会留下备用源的一条
`PRIMARY SUCCEEDED`，主源失败那一半根本不存在。
而 `drills.provider_fallback_drill()` 读的正是 `provider_attempts`。

### 验收进度 · 演练 4 / 6

`PRIMARY_FALLBACK` 已记进账本，证据是 `dr-3caea0f9…` 这次真实同步：

```
eastmoney_security_master  PRIMARY   FAILED_RETRYABLE  (push2 全 502 原文)
sina_security_master       FALLBACK  SUCCEEDED
```

记之前先证明它会红：只有 PRIMARY 成功的 run → False；不存在的 run → False。

### 已知问题 · 周一的 EOD 定时器仍然会失败

`cn.security_master` 不再是瓶颈了，但 `eastmoney_eod` 的 `_HOSTS` 是
`82.push2` / `push2` —— **同一组挂掉的 host**，实测同样全失败。
⇒ 见 `TODO.md` 的 E-4b。
...
### 新增 · 教程第 68 / 69 章（裁定 10）

- **68 · 合一份「已经自测过」的增量包 —— 那 93 条测不到的四件事**
  基线相同时用三方合并；「93 passed」是交付方的口径不是这棵树的结论；
  一条永远被 `importorskip` 跳过的测试在「我跑过了」这句话里和真跑过长得一样；
  🔴 一个专门消灭假绿的闸门自己带了一处 L-13。
- **69 · 让一项从没跑过的演练真的跑起来，以及两处「自己证明自己」**
  判据写好了 ≠ 这项检查跑过，中间隔着一整套没有工具的操作；
  恢复树上用错入口会得到与恢复无关的 PASS；
  🔴 一个「半完成」的条目标成「未开始」，比标成「已完成」更难被发现。

### 🔴 写验证命令时又踩了第 61 章那个坑

第 68 章的验证是「把 dataset 模块的常量改坏 ⇒ 闸门退 1，还原 ⇒ 退 0」。
照第一版跑，**还原之后闸门仍然退 1**。
...
⇒ 验证命令里补上 `__pycache__` 清理，并写明「那不是保险起见，不加就是错的」。
它值得写进教程，因为它**不是一次性的手滑**：同一个坑第 61 章记过，
这次是在写验证命令的时候又踩了一次。
...
> 两个派生数据集的 raw 血缘一直在**自己证明自己**。
> 这条是核对一条「过期 TODO」时翻出来的 —— 它标着「未开始」，
> 实际是**半完成**：进册那一半做了，血缘那一半没做，而没人再看第二眼。

### 修复 · 🔴 派生数据集的 raw 不再是自己的输出

`cn.security.tradability` 旧实现：

```python
raw = json.dumps([item.to_dict() for item in records])
```

**把自己的输出重新序列化当 raw。** 于是 `content_sha256` 是对**输出**算的 ——
回放去校验它，校验的是「我写下的等于我写下的」，与真正的来源
（行情源那次 EOD 响应）毫无关系。而同一次取数里，日线走的是真响应：

> **同一次取数，两个数据集两套口径。**

`cn.market.emotion_close` 是同一个毛病，而且更明显：生产路径上喂给它的那个
dict 是 `decision_client` **自己从股池算出来的计数**（`up.total`、
`broken.total / denom`、`max(streaks)`）—— raw 存的是我们的中间结果，
不是任何人发给我们的字节。

### 新增 · `upstream_artifact_ids` —— 引用上游，不复制

`SnapshotPublishRequest` 新增这个字段，与 `raw_artifacts` **二选一**。
分区的 `raw_artifact_id` 外键直接指向上游那一条。

🔴 **为什么不是「把上游的字节复制一份到自己名下」**：那份 raw 的
`provider_id` 会写成 `derived_biga`，而字节其实是行情源给的 ——
读的人会以为这个 provider 返回过这些东西。**引用，不复制。**
...
| 两种血缘同时给 | 抛 —— 分区上只有一条 `raw_artifact_id` 外键，必然有一个是摆设 |
| 派生发布漏传上游 | 各 dataset 自己抛，**不悄悄退回自造** |

⚠️ 复用既有快照的路径也把血缘带出去（取自既有分区的 `raw_artifact_id`）。
不带的话，「今天重跑」与「今天第一次跑」给下游的东西不一样 ——
而下游会静默退化成自造。

实测（2026-09-26）：跑完 EOD 整包后，`cn.security.tradability` 与
`cn.equity.daily_bars` 的分区指向**同一个** artifact id，
`raw_artifacts` 里不再有 `derived_biga` 的行，盘上也不再有
`data/raw/cn_security_tradability/`。探针 7/7 破坏后红、还原后绿。

### 变更 · TODO 里一条「未开始」其实是「半完成」

`⓪-b 可交易性进册` 写着两个条件（有读取方 + raw 指向同一份响应）。
第一个在 P4-G0 就满足了，第二个一直没做，而条目整体还标着 `[ ]` ——
**读起来像是整件事都没开始**，于是没人再看第二眼。

---

## [0.9.1] - 2026-09-26

> 六项上线演练里**第一项真的跑过了**：`SOURCE_ZIP_REPLAY`。
> 它此前不是「难」，是缺那几步操作的工具 —— 于是三个月里一次都没跑。

### 新增 · `tools/verify/source_bundle.py` —— 把一次决策的字节打包、再摊回一棵树

`export` 按 EvidenceSet 的冻结血缘收集控制面 + 数据面；`restore` 摊回干净目录。
判据本来就有（`drills.py:source_zip_replay_drill`），缺的一直是它前面那几步。

三条硬规矩，每条都对应一种会**通过**的错法：

| 规矩 | 不这么做会怎样 |
|---|---|
| 只带这次决策引用到的字节，不扫整棵 `data/` | 多带的文件让回放在一棵本不该跑通的树上跑通 |
| 库用 `VACUUM INTO`，不是 `cp` | WAL 里未 checkpoint 的事务会被漏掉，而**漏掉不报错** |
| `storage_uri` 是绝对路径就拒绝导出 | 绝对路径搬不动，恢复出来的库仍指着原机器的目录 ⇒ 读到**原树**的字节并 PASS |

### 🔴 实测记下的一处假绿：恢复树上不能用 `bin/biga-data` 跑回放

那个壳启动时 `cd` 回仓库根（为了让默认的 `data/` 相对路径成立），
而分区 URI 按**进程工作目录**解析。2026-09-26 实测：
把恢复树里一个 Parquet **删掉**，经 `bin/biga-data` 跑出来仍然 `passed=True`。

⇒ 演练结果里记下 `cwd` 与 `db_path` —— 这两个值才决定读的是哪棵树。
不自动判定两者算不算同一棵树（目录布局不止一种，猜错会把合法调用判红），
但账本必须回答得出「当初读的是哪棵树」。

### 修复 · `biga-data --db <值> <子命令>` 一直报「未知子命令 \<值\>」

挑位置参数的判据是「不以 `-` 开头」，于是**带值全局标志的值**成了第一个位置参数。

> 它比「报错了」更糟：错误信息把人引向「子命令拼错了」，
> 而子命令是对的 —— 真正的原因是它前面有个带值的全局标志。

而 `--db` 正是把 CLI 指向另一个库的唯一方式，也就是恢复树回放**必须**用的那个标志。
修法：带值的顶层标志只有一份表，**注册到 parser 用的就是它**，跳过值时用的也是它。

### 验收进度

`SOURCE_ZIP_REPLAY` 已记进 `data/phase3_acceptance.jsonl`（真实决策
`BIGA-20260926-001` 的 EvidenceSet，恢复树上跑的）。记之前先证明它会红：
改一个字节 → 红、删掉文件 → 红、还原 → 绿，两种破坏走同一个异常类型。

六项演练现在 **3 / 6**（`RAW_TAMPER` / `DUCKDB_RUNTIME` / `SOURCE_ZIP_REPLAY`）。
剩下三项各卡在不同的硬条件上，见 `TODO.md` 的外部阻塞台账。
...
两版的关键差别不在内容，在**它是怎么写出来的**：v1 照 Phase 2 收口源码重新扫描，
旧草案从理想架构反推。结果是旧草案有两条与现状直接冲突（schema 从 v21 起排 ——
实际已 v22；日历主源写官方端点 —— 实际那条路在本项目部署环境连不通、零生产调用方），
v1 两条都对。
...
v1 独立得出同样结论        9 条
v1 更精确，按它改          4 条
v1 未涉及，本仓库裁定保留  1 条（退出码映射）
```

⚠️ 那 9 条是**独立到达**的 —— 我靠探活查库，v1 靠重扫源码。两条路得出同一个结论，
这件事本身比任何一条结论都值得记：**设计与现状冲突时现状赢，而查现状的成本比
想象中低。**

#### 🔴 ① 注册范围 7 → 2：我判错了

第一版按「库里真实出现过的 7 组」全收，理由是「只收一半就会有两份答案」。
v1 写死 P3-0 只注册现有持久链的两个，其余「在对应迁移 PR 才激活，避免 Registry
成为愿望清单」，并规划了守卫 `no zero-consumer active dataset`。

**v1 是对的，理由正是本仓库自己那条。** 我那个理由把两个问题混成一个：
「系统里有哪些数据集」的答案在各 skill 代码里，Registry 收不收都改变不了；
「Data Platform 管着哪些」才是名册该答的。那 5 个今天是 agent 直连 provider 的
遗留路径 —— 收进来等于让名册**声称管着 5 个它完全没接手的东西**。

⇒ 教训：**名册的范围由「谁在用它回答什么问题」决定，不由「事实上存在多少」决定。**
我当时反复检查的是后者，那个检查再仔细也到不了正确答案。

⇒ 顺带实现了 v1 那条守卫：`DatasetDefinition.consumers` 是**必填**的，
判据是 import 模块 + `hasattr` 断言符号真的在（行为判据，不是字符串扫描）。
空元组当场红 —— **想注册就得先答出谁读它**。

#### 🔴 ② `provider_id` 从站点前缀改成适配器级：关切对、结论错

第一版取 `raw_market_snapshot.source` 的前缀，于是新浪的日线、日历、快讯合成
一个 `sina`，`datasets_of("sina")` 说「sina 挂了影响 3 个数据集」——
而那是三个**可以各自独立挂**的端点。它系统性地**高估影响面**，
而「它挂了会影响什么」正是这份名册的主要用途。

⇒ `provider_id` 与 `providers/` 下的模块同名；原来的关切（别让同一个源有三套
名字）挪到 `ProviderDefinition.source_prefix`，由测试单向钉住。

⇒ 教训：**一个字段的口径要由「用它回答什么问题」倒推，不由「它和哪份既有数据
长得像」倒推。** 我从「库里是什么样」出发，那个起点保证一致性，不保证正确性。

#### ③ dataset id：两处命名 + 一处概念

`cn.index.quote`→`cn.index.realtime_quote`、`cn.sector.rankings`→
`cn.sector.board_snapshot`。第三处不是改名：v1 把**原始涨停/炸板/跌停池**
（observed）与**情绪收盘指标**（derived）拆成两个 dataset，而我注册的那条
内容是前者、名字用了后者 —— 后者今天根本不存在。这正是裁定 16 的
`observed`/`derived` 之分在 dataset 层的体现。

#### ④ `DataStatus` → `DataRunStatus`，`COMPLETE` → `COMPLETED`

v1 另有一个 `DatasetStatus` 描述**快照**状态；我那个描述的是**一次执行**的终态。
两层东西、几个值同名，不改名会在 P3-1 引入 `DatasetStatus` 时变成必须靠上下文
区分的歧义。

#### ⑤ 修掉 `coordinator.py` 的注释漂移

v1 的 gap 表点名了它。核实属实：docstring 写着「现在 `freeze_index_daily`
**还没有生产调用方**」，而编排器早在批 D-II 就接了。
🔴 这类漂移的危害不是读者少知道一件事，是**读者据此做决定**：一份说自己没有
生产调用方的模块，看起来是可以随便改签名的。

#### 🔴 顺带：一条红过的守卫，数据一变就不红了

收窄到 2 个 dataset 后 `validation_providers` 全为空 ⇒ 把 `datasets_of` 里
validation 那一整条判断删掉，探针**照样全绿**。守卫没写错，是数据不再走到那条
分支 ——「当前数据恰好测不到」和「守卫漏了」在结果上一模一样。

⇒ 角色判定抽成纯函数 `uses_provider()`，用**合成的** DatasetDefinition 覆盖三种
角色。仓库里已有同款解法（`test_条数只数被跟踪的文档` 喂的就是合成集合），
不发明第二种写法。

#### 实测

```text
守卫   13 条（+3：零消费方、消费方符号存在、三角色合成测），全部经探针见红
测试   1913 → 1915 条，0 failed
注册   dataset 7 → 2，provider 4 → 3（未激活的随各自 P3-6 迁移 PR 进册）
```

---

### 新增 · Phase 3 · P3-0：Dataset / Provider 两张名册 + `bin/biga-data`

回答一个此前没有地方能回答的问题：**这个系统里有哪些数据集、各自谁在产、
某个源挂了会影响什么。**

```text
src/easyup_biga/data/contracts.py           领域契约 + 状态→退出码
src/easyup_biga/data/registry.py            DATASETS（7 条，唯一手写处）
src/easyup_biga/data/provider_registry.py   PROVIDERS（4 条），关系**派生**
...
参考骨架给 `cn.trading_calendar` 写的是官方源 + 按月切片，仓库文档也这么说。
**探活查库**（开发流程 §1）：

```text
fact_trading_calendar 全部 1326 行的 source = sina:calendar/klc_td_sh
providers/szse.py 有测试、有导出，但零生产调用方
```

官方那条路在本项目的部署环境里连不通，批 L 之后真正在跑的是另一个源。
⇒ 照抄等于注册一个**在这台机器上一行都没产出过**的主源，连它的切片键也
只对那个端点成立。不是骨架写错了、也不是文档写错了 —— 错的是**从没人回头
核对过现状**。

#### 名册收的是「今天真的在跑的全部」，不是「这一期要做的那几个」

只收目标清单里那几个，等于宣布「系统里有哪些数据集」有两份答案：注册表一份、
`raw_market_snapshot` 里的事实一份 —— 而注册表是后来的那份，它会输。
⇒ 按实际出现过的 source 前缀与端点，7 组一个不漏。
还没探活过的（`cn.equity.daily_bars` 等）**不进**：占位符会让人以为这条链
已经有人管了。

#### 三条结构性决定

- **`provider_id` 取 `raw_market_snapshot.source` 的前缀**，不是模块名。
  写模块名，同一个源在源码、库、注册表里就有三个名字
- **「谁支持哪些 dataset」派生不手写。** 它与 dataset 侧的
  primary/fallback/validation 是一对孪生清单（开发流程第五问），
  两处各写一份必然有一天只改了一处，而失败是静默的
- **`TIMEOUT` 退 2 不是 1**（裁定 12），对齐 `tools/verify/_verdict.py` 的
  「2 去查数据、1 去查代码」。`CANCELLED` 退 130，不塞进三态 ——
  「我按了 Ctrl-C」和「代码有 bug」不该长成同一个数字

#### 🔴 探针抓到两条**结构上不可能红**的守卫

唯一性断言原本是 `len(ids) == len(set(ids))`，打在 `DATASET_REGISTRY.values()` 上。
而注册表是 dict 推导建的 —— **重复的 id 早就被静默吃掉了**，`values()` 里
永远不可能有重复。这条断言结构上不可能失败。

最难受的是：同一个函数的 docstring 里写着「重复会静默互相覆盖，**长度对不上
是唯一能看见的痕迹**」—— 道理写对了，断言打在了看不见那个痕迹的地方。L-13。

⇒ 手写的改成元组（`DATASETS` / `PROVIDERS`），dict 另外派生，
守卫比两者的长度差。

> 通用原则：**当一个结构会「静默吸收」某类错误，守卫必须打在被吸收之前那一步。**

#### 🔴 还原了源码，`__pycache__` 里还是坏的

探针把 `TIMEOUT: 2,` 改成 `TIMEOUT: 1,` —— **等长**替换，还原又在同一秒内。
Python 的 `.pyc` 失效判据是 **mtime + size**，两个都没变 ⇒ 缓存被认定仍然有效。
结果：源码 sha256 逐字节一致、探针如实报「已还原」，随后的全量测试却红在一个
**源码里根本不存在的值**上。

⇒ 探针加 `__pycache__` 清理；这条补进 `.claude/skills/dev-workflow/` §3
（那里本来就有「探针自己也会有 bug」一段，这是它的第二个实例）。

#### 守卫判据的方向

「provider_id 与既有 source 前缀同口径」第一版想反向扫 provider 模块里的
`"<x>:` 字面量 —— 那既漏又噪：东财那个前缀根本不在它的 provider 模块里
（在各 skill 里拼），而 provider 目录下另有 `"https:` 之类噪音前缀。
⇒ 改成**注册表 → 源码**的单向存在性，精确命中要防的那件事。

#### 两处与设计文档的里程碑表不同（已回写设计文档）

- `bin/biga-data list` 从 P3-1 **拉前**到 P3-0：名册自己也得有消费方，
  否则它在 P3-1 之前就是一条 L-1 死配置 —— 而同一份设计刚用这条理由
  拒绝了 `required_datasets` 提前进场
- 注册表的范围见上

⚠️ 顺带修正裁定 4 的一处表述：`cn.market.breadth` 不是 Phase 3 才拆出来的，
它本来就是一个独立的生产数据集。裁定 4 真正要防的是把它绑给 emotion 消费，
那条仍然成立。

#### 实测

```text
测试   1891 → 1913 条（+13 条守卫 +8 条文档参数化），0 failed
       ⚠️ 一度写成 1908 —— 在脏树上同步的，当时教程新章节还未跟踪、它的 case 被守卫排除了。
       正确顺序是先 commit 再同步再 amend（已补进 CLAUDE.md）
探针   10 / 10 全部见红
审查   audit_public.sh --worktree 十一项全绿
```


### 新增 · Phase 3 设计：数据平台地基 + 外部设计包的 14 条适配裁定

收到一份外部 Phase 3 设计包（数据平台地基，7 份文档 + 4 份参考代码骨架）。
方向与本仓库**高度一致** —— 它提的四个存储平面（SQLite / Raw 文件 / Parquet /
DuckDB）`architecture.md` §5.1 早就逐项写死了选型**和触发条件**。所以它不是一份
要重新论证的新架构，而是把 §5.1 里「选型已定、未建」的三个平面真的建起来的施工图。

🔴 **但它是按通用工程写的，不知道本仓库的守卫、裁定与既有实现。**
逐条核对出 **14 处**会撞，其中 **5 处会让现有守卫真的报红**：

```text
schema 从 v21 起排         —— 实测 SCHEMA_VERSION = 22，v21/v22 都已占用
biga data run <job>       —— bin/biga 是纯 passthrough，openclaw 没有 data 子命令
eod-daily-bars.timer      —— 单元名是共享命名空间（R-2），isolation.py 会红
涨跌家数进 emotion dataset —— 破裁定 15，全仓静态扫描盯着
四份 phase-3-*.md          —— 破「一个阶段一份设计文档」
```

⇒ 新增 `docs/design/phase-3-data-platform.md`，把 14 条一次裁定完。
**开工时以它为准，不以外部包为准。**

#### 为什么是「裁定单」而不是直接照着做

照抄外部材料的具体数值，本仓库已经栽过一次：**v20 撤 v18** —— 当时照抄评审
建议的索引名、没先 grep 一遍既有约束，造出一条与 `ux_evidence_set_per_run`
逐字相同的索引。照抄它给的 schema 版本号是**同一个动作**。

#### 为什么裁定单必须自包含

`.gitignore` 里 `docs/external/*` 整个被排除（现在被跟踪的三份是当初显式
force-add 的）⇒ 那个包**任何人 clone 下来都看不到**。所以裁定单里每一条都先
复述外部包说了什么，再给裁定，而不是写「见外部包 §N」。
指向一个 clone 不出来的东西，和「一个 clone 不出来的验收数字」是同一个毛病。

⚠️ 写的时候当场被守卫抓了一次：`test_设计文档点名的文件必须真实存在` 红在
「点名了外部包里的一份 ADR」——**那正是这一节要防的事**，而我在写这一节的
同一份文档里犯了它。已改成描述而不是路径。

#### 两条实质性裁定

**pyarrow 进 `[project.dependencies]`。** §5.1 把「全市场 EOD 第一次建的时候就该用
Parquet，不是数据长大了才搬」写死了 ⇒ P3-4 必须落 Parquet。
代价是「clone 下来就能跑」变成「先联网装 pyarrow」，对一份同时是开源教程的
仓库有实感影响，所以记在这里。
⚠️ 顺带纠正一个易错点：`dependencies = []` 那道守卫钉的是**「声明 ⇔ 代码双向
一致」**，不是「零依赖」——加 pyarrow 并声明它，守卫照样绿。
**不**走 optional-dependencies + 降级：那会让同一个 dataset 有两套存储口径、
两条读路径，正是 L-3 的形状，而降级路径几乎不会被测到。

**Phase 3 = 数据平台 + 第一条 cron**，`discipline` 与独立飞书应用推到 Phase 3b。
路线图原文是四项一起做。收窄的理由：EOD job 的 timer 天然就是第一条调度，
路线图那条出口条件「每条 cron 都有被证明的消费方」对它直接适用、且能真的验；
而 `discipline` 推迟的原始理由（输入源不存在，硬建只能编）**至今成立**。
🔴 这是一次明确的范围收窄，不是遗漏 —— 路线图那一行要同步改，否则它会静默
变成一张对不上的表（Phase 2 条件 4「达成了四天没人知道」就是这个形状）。

#### 另外三条值得单独记

- **`required_datasets` 跟 Snapshot Resolver 一起进 P3-7**，不在 P3-0。
  `domain/registry.py` 的 docstring 专门写过当时为什么不装它：「装一个没有消费方
  的字段就是一条 L-1 死配置」。Phase 3 解除了当时的理由，但消费方要到 P3-7 才有 ——
  按外部包的顺序，中间五个里程碑里它就是死配置
- **DuckDB 推迟到选股闭环。** §5.1 写明它不独立触发（是查 Parquet 的引擎）。
  Phase 3 里没有选股消费方，现在建就是 L-1 零消费方
- **EvidenceSet Manifest 缺版本字段 ⇒ 按 v1 读。** 实测现库 12 行 manifest 的键是
  `frozen_bars/kind/symbols`，没有 `manifest_version`；而外部包又规定「未知状态
  fail closed」⇒ 照字面实现，这 12 张卡的 replay 全挂。验收判据不是「新卡能读」，
  是**那 12 张老卡仍然逐张通过**

#### 范围之外但必须记的一条

第一批 dataset **不含 news**。而 Phase 2 出口条件 5 卡着的「盘后 198s 超预算」，
根因正是收盘后快讯量翻倍 ⇒ 别把 P3-6（emotion 不再联网）当成顺带解决延迟问题
记进 Phase 3 的账，它不解决。
...
**起因是一次真实事故**：2026-09-25（中秋）在**休市日**出了一张卡
（`BIGA-20260925-001`）。六个 agent 里 `news` 按「今天」报交易日 `20260925`，
日线类报 `20260924`，`risk` 因交易日不一致判 `stance=无法判定`——
而 `20260925` 根本不是交易日。**系统不知道自己在休市日运行。**

根因：`fact_trading_calendar` **0 行**。原生产方走深交所官方端点，本机连不通它
（`szse.py` 模块头早就写明了这个网络事实，也写明「退化方向安全」）——
退化方向确实安全，但代价是 `market_is_open()` 恒走 weekday 回退，
对任何工作日上的法定节假日都答「开市」。

⚠️ 顺带纠正一处文档与实测不符：`CLAUDE.md` 写着「v15 …… `market_is_open`
**认节假日了**」。在这台机器上**不成立**——表是空的。描述的是能力，不是状态。
与 README 徽章停在一个早就不成立的数字上，是同一个形状。

**为什么不能用指数日线反推**

盘中日线**不含当天** bar（这正是 `phase-2-specialists.md` §3.10 记的
「盘中日线类报上一交易日」）。所以日线只能回答过去，回答不了「今天开不开」——
而那恰恰是出问题的那个问题。⇒ 必须要**前瞻**日历。

**探测了五个源，可达的前瞻源只有一个**

```text
新浪 klc_td_sh.txt     ✅ 可达   1990-12-19 → 次年年末（含已公布排期）   ❌ 压缩编码
eastmoney 指数日线     ✅ 可达   只到上一交易日                          ✅ 干净 JSON
深交所官方             ❌ 超时
上交所 commonQuery     ❌ 超时 / System Error
eastmoney 各 RPT_* 日历 ✅ 可达   报表不存在
```

**新浪那份是压缩编码的**——508 字节压 8796 个日期。通行做法是引一个封装库，
它把一段 ~17KB 的混淆 JS 丢进 JS 引擎跑。两条都不合适：引库要多拖 JS 引擎与
DataFrame 两个重依赖，而我们只要一列日期；把那 17KB 第三方混淆 JS 抄进本仓库
则是授权与可维护性问题（本仓库 Apache-2.0、公开）。

⇒ **读懂它，自己重写。** 新增 `src/easyup_biga/providers/sina_calendar.py`：
位流读取器（6 bit/符号、LSB-first）+ 三个原语（取位 / 取定宽字段 / 带符号一元码）
+ 游程解码。**不是「差不多对」**：拿离线 fixture 解出的 8796 个日期与通行实现的
解码结果**逐一比对，完全相同**（对方另外手工补了一天 1992-05-04，是它自己加的，
不在编码里）。

🔴 **这个编码不是日历专用**，是新浪的一族压缩序列（日 OHLCV / 分时 / 收盘序列
……），由流首 12 bit 的 format id 分发，共六支。**只实现日历那一支（139）**——
其余五支现在没有消费方，预先写出来就是 L-1。撞到别的支会抛错**并报出那个 id**，
位流原语已被日历这一支逐字节证明过，将来加一支是局部改动。

### 变更 · `market_is_open()` 变成三层（带覆盖区间守卫）

```text
1  周末短路
2  fact_trading_calendar（bin/biga-calendar 刷新，含未来排期）
3  硬编码节假日兜底 —— **两端都有覆盖区间守卫**
4  工作日 ⇒ 当作可能开市（R-3 的安全方向，未变）
```

🔴 **第 3 层的区间守卫是这次唯一真正的新东西。** 硬编码假期表会烂，而且是
**静默**地烂：表停更之后，每个新节假日都被悄悄判成交易日。守卫把它换成
「区间外一律弃权（返回 None）」——绝不乐观地答 True。

⚠️ **两端都要**。第一版只写了上界，于是这一层对 `_COVER_THROUGH` 之前的任何
一天都敢答，而表只列了导出那天之后的假期——2026-01-01（元旦）被自信地判成
交易日。「不在假期表里 ⇒ 是交易日」这句话，只有在**表确实覆盖那一天**时才成立。

### 修复 · 手抄假期表当场出错（留作判据的依据）

`_HOLIDAYS` 第一版是**手抄**的，照着另一份同类系统的 2026 假期表抄，把国庆后的
**复市日 `20261008` 抄成了休市日**（那张表把中秋标成「与国庆合并」，
而 2026 中秋在 9 月 25 日）。权威数据说 10-08 是交易日。

🔴 这件事发生在写完「硬编码表会烂」那段注释之后的**十分钟内**。
⇒ 判据只能是「从数据导出」，不能是「仔细一点」。新增
`tools/verify/dump_holiday_fallback.py`（同一个解码器、同一份数据导出），
并有一条测试把表与权威日历**逐项对平**。

### 新增 · 到期提醒，把「静默地烂」换成「按期响亮地提醒」

`test_兜底表到期前会提醒` 在 `_COVER_THROUGH` 前 45 天开始变红，报错里直接给出
要跑的两条命令。⚠️ **它按期变红是用途，不是故障。**

### 修复 · 编排器不记耗时，端到端验收因此永远判不了

`card_ops.synthesize()` 的 `elapsed_ms` 编排器**一直没传**，卡上恒为 0。
后果不是少个字段：`phase1_acceptance` 第 8 项报「无法精确定界这次决策的时间窗」，
出口条件 5（按实测重推延迟预算）因此没有可引用的数字，只能每次去 trajectory 反推。
2026-09-25 真机验收实测撞到。

改为 `time.monotonic()` 差值（不是 `now_cn()` 之差——墙钟测的是**时长**，
系统时钟可能被 NTP 拨动；这与「写库时刻一律北京时间」不冲突，那条管的是**时刻**）。

### 变更 · Phase 2 出口条件 4 已达成（而且四天前就达成了）

直接查库：真实缺数据代码（带点号的，不是 `xxx agent 尚未上线` 那种结构性占位）
跨 **09-21/22/23/24 四天**、44 张卡。文档里那句「🔶 全在同一天」是 09-21 当天
写下的快照，之后四天的数据早就填满了——**没人回来重新评估**。

🔴 值得记的不是「达成了」，是「达成了四天没人知道」。⇒ 新增
`tools/verify/exit_conditions.py`，把能由程序判定的那两条从手写状态表换成可跑的。

### 验证 · Live Acceptance 重跑（v0.3.6/v0.3.7 第一次走真实路径）

`BIGA-20260925-001`，rc=0，76.8s / $0.98：

```text
1 Decision / 1 Run / 1 EvidenceSet         ✅
agent_runs 6/6 provenance_mode='online'    ✅  v19/v21 第一次落到真实数据上
6 条 VerdictRef run_id 全等于卡的 run_id    ✅
spawn_check --run-id（逐行严格）            ✅  rc=0
replay --check / readback_check             ✅
Stage 1 并行：最小重叠 17.7s                ✅  墙钟 55.6s vs 个体之和 184.1s
phase1_acceptance                           PASS 6 · FAIL 0 · PENDING 3
```

三轮静态审查之后第一次真跑，**新代码零回归**。

### 已知未完成

```text
Phase 2 条件 3（真实否决）  等一个命中 risk 六条阈值之一的交易日。不是开发问题
Phase 2 条件 5（延迟预算）  elapsed_ms 已修，等下一次盘中运行取数
v1-architecture-baseline    仍未重新冻结，README 标 🔶
```

测试 1875 条（Hermetic 口径）/ 1908 passed / 0 failed；source ZIP 1873 / 0 failed。
...
**README 全文重写（Final Gate 第 2 条：README 与实际状态一致）**

采纳外部评审组提供的结构改写稿，同时保留项目当前真实数据（1817 条测试、schema v19、
v0.3.1–v0.3.4 已完成的所有 Hardening 项）。

主要结构变化：
- 新增「BigA 的架构主张」——把 Pipeline 示意图和 10 条设计主张放在最显眼的位置，
  说清楚"BigA 不是把 Agent 名字写进 Prompt 的 Demo"。
- 新增「当前不是在做什么」——列明当前阶段的明确边界，防止读者对功能范围有错误预期。
- 「OpenClaw 与 BigA 的边界」升为独立节（原来散在正文中）。
- 「设计地基」从 6 条扩充到 8 条（新增「通知不等于业务成功」「历史不可覆盖」）。
- 「当前实现状态」表更新：v0.3.1–v0.3.4 完成的所有 Hardening 项标为 ✅；
  `status` 徽章改为 `architecture baseline hardening`。
- 新增「BigA 与其他项目的关系」——说明 BigA 借鉴的是什么、自己拥有的是什么。
- 新增「快速开始」的 `bin/biga-reap` 命令。
- schema 版本和测试条数与实际一致（v19 / 1817）。
...
出卡，10 分钟内又用飞书 `/card` 触发第二次——第二次被预算闸门的 `MIN_GAP_SEC`
拦下（390s vs 400s，差 10 秒），但飞书那一侧收到的是"收到，正在出卡……不用等
也不用重发"。

根因：`inbound.py::accept_trigger()` 的 ACK 在 `launcher()` 成功把 `systemd-run`
瞬态单元排上去之后就发了；而预算闸门是在那个单元**内部**（`bin/biga-card`）才
执行的——`launcher()` 只保证"排上去了"，不等它跑完。单元几乎立刻自己因为闸门
退出（exit code 3），飞书那一侧再没有任何后续。用户收到一句关于未来的承诺，
而那个承诺在发出的同一秒已经不成立了。

⇒ `accept_trigger()` 在决定拉不拉起之前，自己先问一遍 `check_budget()`
（`exclude_decision_id` 排掉刚占的号，否则「跟自己比 gap 恒为 0」——`bin/
biga-card` 自己那道检查早年间已经踩过这个坑）。闸门会拒就不拉起、直接说真话，
省下一次必然自杀的 `systemd-run` + Python 解释器开销。

🔴 **这不是新增一道闸门**——真正的拦截仍然只在 `bin/biga-card` 内部发生一次；
这里问一遍纯粹是为了让 ACK 说真话，`check_budget()` 本身只读不写，问错/漏问
不会漏放任何东西。

新增 3 条测试 + 改了 1 条既有测试（它背靠背触发两个不同 trigger，恰好会撞上
新加的这层检查，而它原本要验证的是"不同 trigger 不共用一个决策"，跟预算时序
是两件事——短路掉闸门让测试回到它本来的焦点）。1 处 sabotage 验证会红。
...
🔴 **开工提示词给的判据对测试文件不成立**，这是本批最重要的发现。原文是「删除前
先跑一次它所在的测试确认能跑，删除后再跑一次确认仍然能跑」。但 `sys.path.insert`
是**进程级全局副作用**，而 pytest 把所有测试模块导进**同一个进程**。实测（都清空
`pythonpath`）：`test_store.py` 单独跑 `ModuleNotFoundError`，加上字母序第一个自举
的文件（`test_agent_registry.py`）就 **70 条全过** —— 而 `test_store.py` 自己
**一行 `sys.path.insert` 都没有**。

⇒ 删一处再跑全量**必然全绿**，因为别处兜住了。那个判据测的是**这一堆**的性质，
不是**这一行**的性质。真正的判据是**单文件、独立进程**（48 个改动文件逐个验过）。
与第 43 章（monkeypatch 依赖收集顺序）、第 51 章（pytest 路更宽测不出）同形，第三次。

**分界线是「谁在跑它」**，且经过核实不是假设：`tests/` 下**没有任何 `__main__`
入口**、**没有任何调用方**用 `python3 tests/...` ⇒ 只经 pytest，`pythonpath =
["skills","src","."]` 覆盖。而 `skills/`·`tools/`·`deploy/` 下那 42 处是
`python3 x.py` 直接跑或子进程拉起的，**没有 `pythonpath`** ⇒ 全部承重。

`tests/` 84 处再拆：删 56（目标 `skills`/`src`/`tests`，前两条 pythonpath 挂好、
第三条 pytest 自己会塞进 `sys.path[0]`）、留 28（目标 `tools/verify`·
`deploy/openclaw`·各 skill `scripts/` —— **不在 pythonpath 上**，删了当场失败）。

⚠️ **剩下 69 处删不掉，理由写进 `TODO.md`**：它们挂仓库根的 `skills/`，而全仓还有
**233 处**在用旧名字 import，`skills/` 又被批 U-II 明确排除在 pip 包之外（三条理由，
含 R-2）。要删就得先把 233 处迁到 `easyup_biga.*` —— 那等于**给 H-I 的薄壳定退役**，
是独立的架构决定，不该作为「删几个路径操作」的副产品夹带进来。

### ✨ 新增 · `tests/test_test_path_bootstrap.py`（6 条）：冗余路径操作不许回来

判据是**解析后的路径**，不是行的形状。探针：把一处删掉的加回去 ⇒ 守卫报红，
而 `test_store.py` 自己**仍然 47 条全绿** —— 又一次证明 pytest 看不见这件事。

🔴 **同一批里按字符串形状分类栽了三次**，值得单独记下来：

1. 第一次分类按「行里有没有 `skills` 这个词」分，于是
   `skills/decision-card/scripts/budget.py` 里的 `parents[2]`（解析出来**正是**
   `skills/`）被分进了「安全」那堆；
2. 清理孤儿 import 时，ruff 同时给了文件名**和行号**，我却用文件名去删「长得像
   `import sys` 的那一行」—— 而 ruff 指的是**函数体内第 136 行**那个。结果删掉了
   顶层那个真正在用的，**3 条测试当场失败**。工具把精确行号交到手上了，是我自己
   换成了模糊匹配；
3. 守卫第一版按「行里有没有 `"scripts"`」判，于是
   `str(REPO / "skills/market-calc/scripts")`（整条路径写在**一个**字面量里）
   没命中，被误判成冗余。

修法统一成真的解析：字面量按 `/` 拆开拼路径、`parents[n]` 按文件自身位置算出来、
动态目标（`str(d)`）判「不覆盖」并写明**为什么那不是 fail-open**——
动态拼出来的目标按构造不可能恰好是那三个静态根之一。

孤儿 import 用批 U-IV 刚接上的 ruff `F401` 找，但要取**删除前后的差集**：
仓库本来就有 25 条 F401，一把 `--fix` 会把不相关的历史违规一起改掉。差集正好 24 条。
...
薄壳让**存量调用方**一字不改 —— 那一批达成了它的判据。但它没搬「包**内部**
互相怎么导入」：`persistence`/`providers`/`application` 里仍写着
`from _contract import ...`，靠 `pyproject.toml` 的
`pythonpath = ["skills", "src", "."]` **同时**挂两条路才解析得开。

**为什么这不是「不优雅」而是缺陷**：它在全量回归下完全无感。实测（本批 G-1 探针）
把 `persistence/db.py` 一行改回旧写法，`tests/test_store.py` **47 条全绿**；
同一时刻只挂 `src/` 的隔离 import 当场 `ModuleNotFoundError: No module named
'_contract'`。测试环境的路比真实安装环境宽 ⇒ 全绿只证明「在宽路上能跑」，
而差别是以**通过**的形式出现的 —— 红线 R-3 说的静默 fail-open。

- **9 个文件、13 行**改成 `from easyup_biga.{domain,persistence,providers} import ...`
  （`persistence` 3 处 / `providers` 5 处 / `application` 1 处）。一次改完不分批 ——
  分批等于同一件事做两次，且中途状态更难判断「改没改全」（`TODO.md` 早就定死的方案）。
  ⚠️ 开工提示词写的是「只改 3 个文件」，那是把「3 个**包**」记成了「3 个文件」；
  范围结论（哪三个包）是对的，数字不是。
- **删掉 `providers/tradetime.py` 的 `sys.path.insert(0, 仓库根/"skills")`**。
  🔴 它不属于批 U-III：U-III 要逐一判断的是「这行现在还有没有用」，
  而这一处的**唯一消费方就是同一次编辑删掉的那行**（原注释自己写着「为了让
  `from _contract import` 经薄壳解析」），不需要判断。留着它只会留下一条
  **指向已不存在机制**的注释 —— 注释说谎比没有注释更糟。剩余 81 处仍归 U-III。
  连带删掉同文件的 `import sys`（自举一走就无用）与 `# noqa: E402`（那个豁免的
  存在前提是「import 出现在代码之后」，前提没了）。`import pathlib` **保留** ——
  它还在函数签名的类型注解里用着。
- **4 处文档字符串更正**：各包 `__init__.py` 的「用法」示例原本只给旧路径
  `from _contract import ...`。包一旦能在**没有 `skills/`** 的环境里单独装起来
  （批 U-II 的验收场景），那行示例就是错的。改成给绝对导入、并注明薄壳对存量
  调用方仍然等价。

**对外接口一步没动**：`skills/` 下的薄壳全部保持原样，H-I 定下的「既有导入一字
不改」的承诺仍然成立，并由验证清单第 4 条单独钉住。

### ✨ 新增 · `tests/test_package_self_contained.py`（7 条）：包自足的常驻判据

本批真正的交付物不是那 13 行 import —— import 可以再写坏，判据在才会当场红。

- **判据是 AST，不是 grep。** 开工提示词给的 `^from _\|^import _` 拿来当守卫
  两头都不准：**误报**文档字符串里的用法示例（4 处）与相对导入的下划线兄弟模块
  （`from ._freeze import ...`，`module` 就是 `_freeze`，本仓库真实写法）；
  **漏报**函数内惰性 import（行首有缩进）与 `__import__("_contract")`
  （本仓库在裸 sqlite3 那道守卫上**已经被实测绕过一次**，见 `_INDIRECT_IMPORT`）。
  四种形状 grep 错三漏二。换成 AST 后问的是「这里真的发生了一次跨包导入吗」，
  `ast.walk` 覆盖函数体内、`ImportFrom.level == 0` 天然排除相对导入。
- 🔴 **惰性 import 单独一条测试，不是冗余。** 惰性的定义就是「导入模块时不执行」，
  模块级 import 测试走到 `import easyup_biga.providers` 就结束了。sabotage 量过：
  只把 `tradetime.py` 惰性那行改回旧写法，`test_只挂src时五个包都能独立import`
  **全绿**，只有 AST 那条和「真把函数调起来」那条报红。
- **三条反向自检**，各堵一个「因为什么都没查而全绿」：扫描范围非空且覆盖五个子包 /
  四种违规形状都抓得到 / 🔴 **隔离环境里 `skills` 确实不可达**。最后一条最容易漏 ——
  少了它，隔离测试有可能因为 `skills/` 其实还够得着而全绿，那时它验证的是一个恒真命题。

探针记录（G-1，两处 sabotage 均已还原）：顶层 import 改回旧写法 ⇒ 3 条红；
只改惰性那行 ⇒ 2 条红、模块级那条全绿（这一条正是上面那段结论的来源）。

### 🐛 修复 · 测试条数徽章 1642 → 1645（上一个提交加了文档没跑同步）

`02aba1a` 往 `docs/guide/` 加了一份开工提示词。`tests/test_docs_convention.py`
有**三条检查按 `docs/**/*.md` 参数化**，每多一份文档就多 3 条 case ——
1642 + 3 = 1645，而那个提交没跑 `sync_test_count.sh`，主分支上 pytest 因此不全绿。

单独一个提交修，不捎在功能提交里：CLAUDE.md 那条纪律要的是**徽章始终描述一个
真被测过的提交**（裁定 14 真正想防的那件事），而主分支是公开仓库访客唯一会看的分支。

怎么确认这 3 条真的来自那份新文档、而不是别处漏掉的测试：在 `02aba1a~1` 的
**干净 worktree** 里跑全量得到 1642 且全绿；再对一遍工作区里 18 份未跟踪文档
贡献的 54 条（`405 - 348 = 57 = 54 + 3`）。三个数字对得上才敢下结论 ——
「基线红着也能往下做」和「这红跟我无关」是两句不同的话，只有后者需要被证明。

### 🐛 修复 · 裁定 16 批 5：空列表不等于安全

**`risk.tripped_thresholds` 把未知显示成了安全。** 上游一个阈值字段都没有时，
比对循环一次都不进，`tripped=[]` 照样上卡 —— 而卡面上的「被触发的风险阈值：（空）」
只有一个读法：**比过了，没有触发**。实际是「没东西可比」。

> R-3 最坏的形状不是「报了个 UNKNOWN」，而是**把未知显示成安全**。
> 前者读者会去追问，后者读者会放心。

⇒ 一个可比字段都没有时**不产出该字段**，改进一条 missing
（`risk.thresholds.nothing_to_check`）。判据用「输入证据在不在」而不是
「字段在不在 result 里」—— 两者在这里是同一件事，而用前者顺带保证了
产出的那一条一定说得出自己的出处。

⚠️ 改完 **20 条编排器测试变红**（risk 转 UNKNOWN 与夹具的 stance 冲突）。
第一反应是「改激进了」，**先去量**：生产 23 次决策里 **22 次**上游有 3–4 个
可比字段，「一个都没有」只出现过 1 次。所以那是**真实的退化场景**，
risk 转 UNKNOWN 恰恰是对的；不现实的是夹具（上游桩只给了一个 `<agent>_x`）。
已把两处夹具补成现实形态。

> 一个改动让大量测试变红时，先分清是**行为错了**还是**夹具不现实**。
> 判据不在代码里，在生产数据里。顺着「测试红了就是改错了」退回去，
> 留下的就是那个把未知显示成安全的洞。

### 🔀 变更 · 契约铁律扩到 `observed`

批 3 的「必须说得出出自什么」只管 `derived`。但「我观察到的」如果指不回任何一份
响应，它和「我编的」在卡面上**长得一模一样** —— 而卡面正是人做决策的地方。

之所以现在才做得了：批 4 的 `derived_from` 给了两个跨源字段
（`market.trade_date` / `emotion.trade_date`）表达出处的方式。在那之前收紧这条
只会逼出假数据。

> 一条规则「现在能不能收紧」，取决于**被约束的对象有没有合法的满足方式**。
> 没有出路时收紧规则，收到的不是合规，是伪造。

实测代价：收紧后全量只红 8 条，**全部在测试夹具里，skill 与生产路径零失败**。
`parameter` 仍不受约束（参数本来就没有数据出处），历史证据 `kind=None` 亦然。

新增探针 5 条，3 处 sabotage 全部验证会红。详见教程第 57 章。

### ✨ 新增 · 裁定 16 批 4：一个字段装下四种出处，E 节收官

**`OriginRef(kind, ref)` 取代 `input_evidence_ids`。** 后者只能说一句话
「我是从**另几条证据**算出来的」，而剩下待接线的字段里有三种它根本说不了：

| 字段 | 真实依据 |
|---|---|
| `risk.coverage_ratio` 等 4 个元数据类 | **哪些 verdict 到场了**（不是任何证据的值）|
| `market.volume_total` 等跨源聚合 | 沪深**两份** raw 快照 |
| `news.market_open` | **交易日历 fact** 的那一行 |

硬塞进去都会变成「说了一句不成立的话」—— 比不说更糟，因为下一个人会以为链是通的。
⇒ 四种来源：`raw` / `evidence` / `verdict` / `fact`，配四个构造器。

🔴 **结构化，不是 `"verdict:123"` 前缀字符串**：那要消费方解析字符串形状才知道
这是什么，正是 L-13。本仓库刚用两批把 `derived:` 那套收拾干净，不该原地再种一个。

⚠️ 校验**按类别分开**：只有 `raw` / `evidence` 的 `ref` 是内容哈希，
`verdict` 是行号、`fact` 是定位串 —— 统一按 sha256 校验会把后两类全部误杀。

**risk 11 个字段全部接线**，按裁定 16 的粒度：值类 4（引支撑那个字段的证据）/
全量类 3（引上游全部证据）/ 元数据类 4（引 verdict）。

**批 3 留的 6 处 `kind=None` 全部消化**，全仓再无「尚未归类」的调用点，
并加 AST 扫描钉死。

> 留白要带**到期日**。一个写明理由的 `None` 是诚实的；理由消失后还留着的，
> 就退化成「懒得想」的挡箭牌。让守卫替你记住到期日，别指望下一个人读得到注释。

### 🔀 变更 · 冻结向量随字段集更新（并记下为什么这次允许）

字段集从 `input_evidence_ids` 换成 `derived_from` ⇒ 全部 `evidence_id` 变化，
冻结向量应声报红 —— **这正是它该红的时候**。

更新之前回答了批 1 写下的那句话「存量引用怎么办」，答案是量出来的：
**生产库 3152 条证据里带 `evidence_id` / `derived_from` / `kind` 的是 0 条**
（批 1–4 从未在生产跑过）⇒ 没有任何存量引用会被打断。
理由连同数字写进了向量文件本身，免得下次有人照着「上次不也改了」放行。

### 已知问题

- **`tripped_thresholds` 的空列表是 R-3 形状**：上游一个阈值字段都没有时，
  `[]` 读起来像「比过了，没触发」，实际是「没东西可比」。本批先如实标成未归类
  （不声称 derived），把它变成一条 missing 是独立的一块，已记进 `TODO.md`。
- **`observed ⇒ 必须有 raw_hash` 仍不能 enforce**：直接证据缺 `raw_hash` 的比例
  最后实测（09-24）还有 13.8%，在收敛但仍在产生。

新增探针 18 条，5 处 sabotage —— 其中一处**补出了一条从来不存在的守卫**
（`OriginRef.kind` 的校验写下来之后从没有测试守着）。详见教程第 55 章。

### ✨ 新增 · 裁定 16 批 3：证据说清自己是什么 + 派生血缘接线

**契约铁律落地**：`kind="derived"` ⇒ 必须有 `raw_hash` **或** `input_evidence_ids`。

🔴 规则**不是**「派生一律要 inputs」。technical 的 `ma5` 是从**整份 K 线**算出来的，
批 2 之后它的 `raw_hash` 已经完整回答了出处 —— 再要一串 evidence id 只能硬编，
那正是裁定 16 禁止的「硬凑」。⇒ 两条出路，占其一即可：
单一来源走 `raw_hash`，从别的值算出来走 `input_evidence_ids`。

> 铁律要按**证据实际长什么样**来定，不是按「哪条规则听起来更严」。
> 一条覆盖不到真实形态的严规则，逼出来的只会是假数据。

**`input_ids_for`**：写字段名，解析成 `evidence_id`；字段不在场**直接抛**。
静默跳过会产出一条「输入列表不完整」的派生证据 —— 它看起来声明过血缘、
实际漏了一截，比完全没声明更难发现。

**五个 skill 的 `add()` 系列加 `kind`，且不给默认值。** 给它默认值，
「忘了想这条是什么」和「想过了，就是这个」写出来一模一样，
而裁定 16 要的恰恰是让人想一遍。41 个调用点逐个声明。

接完之后 emotion 的血缘链是真的两跳：
`emotion_score ← limit_up_count, max_streak, broken_rate ← limit_up_count, broken_board_count`

⚠️ **留白不是漏做**：6 个字段故意留 `kind=None`（合法的「尚未归类」）并在调用点写明原因
—— `volume_total`（沪深之和）、`volume_ma20`（还用了 20 天历史序列）、`volume_ratio`、
`trade_date`、`board_counts`（行业榜+概念榜合计）、`market_open`（交易日历+时钟）。
共同点是 **Evidence 今天只能记一个 `raw_hash`，而它们有多个来源**。
编一组能过检的 inputs 会让下一个人以为这条链是通的。批 4 要么补 per-symbol 证据、
要么让 Evidence 支持多来源引用。

### 🐛 修复 · emotion 的指纹计算被 `store` 开关门控

铁律一上线就抓到：`self.hashes[...] = raw_text_sha256(...)` 写在 `if self.store:` **里面**，
于是 `--no-store` 跑出来的证据**全都没有 `raw_hash`** —— 同一段代码、同一份数据，
**可追溯性却取决于一个与追溯无关的开关**。

market / sector / news 三个 skill 本来就是先算哈希再判 store，emotion 是唯一的例外。

> 一个属性属于**数据**还是属于**这次运行的选项**，要分清。哈希是那份响应的属性，
> 与「我们这次存不存盘」无关。把前者写进后者的分支里，就会得到
> 「同样的输入、不同的可追溯性」。

### 🔀 变更 · 一条钉整行源码的判据放宽

E-19 留下的 `test_news自己那个同名字段不受影响` 断言的是**一整行源码**，
批 3 给 `add()` 加参数时它就红了。它的意图是「news 的同名字段没被顺手统一掉」，
判据因此放宽到「字段名 + 标签」这对组合 —— 源码级判据要钉**意图所在的 token**，
不是整行；钉整行等于给所有无关改动埋一颗雷。

新增探针 21 条，4 处 sabotage 全部验证会红。详见教程第 53 章。

### 🔀 变更 · 裁定 16 批 2：溯源归属收成一份，并改对了写宽的规则

**五份口径收成一份**（`domain/provenance.py`）。在它之前，「这条证据出自哪份
原始响应」这个判断在 market / emotion / sector / technical / news **各写了一遍，
且五份写法都不一样** —— 写法不一致本身就说明它们已经各自漂过。
判断 L-3 的标准不是「代码长得像」，是**改了一处另几处会不会跟着对**。

**政策改动：规则的理由对，范围写宽了。** 五份的共同规则是
「`source` 以 `derived:` 开头 ⇒ 一律丢弃溯源」，注释里的理由无可挑剔
（「不硬凑：凑出来的溯源比没有溯源更糟」）。但很多派生值的 source
**本身就点名了单一来源**（`derived:sina:kline/sh000001`），剥掉前缀就是一个
真实表键 —— 它确实出自那一份响应，指回去是如实记录，不是硬凑。

实测生产库 1721 条派生证据里 **770 条**属于此类，溯源信息一直都在、被白白丢掉。
改动后离线实测：technical 的派生证据从 0/12 带 raw_hash 变成 **12/12**。

🔴 **匹配方向只能有一个**：source 比表键**更具体**（子路径）⇒ 命中；
**更泛** ⇒ 不命中。第一版写成「只认精确命中」，被既有测试
`test_market单源证据都有raw_hash` 当场抓到 —— `turnover_sh` 的 source 是
`tencent:quote/sh000001`、表键是 `tencent:quote`，前缀匹配在那里是承重的。
反方向（表键以 source 为前缀也算）看着更宽容，实际是把「没法确定是哪一份」
翻译成「就算是某一份吧」，**R-3 的形状**。

### 🐛 修复 · 更正批 1 的一处病因判断

批 1 的 TODO / commit 写着：`market` 的泛化 source `sina:kline` 匹配不上表键
`sina:kline/sh000001`，「病因已定位」。**机制描述是对的，结论是错的。**

`volume_total = sum(d.last.volume for d in c.daily.values())` —— **沪深两市之和**，
它真的没有单一来源，泛化 source 与 `raw_hash=None` 都是诚实的。
当初点名的 4 个「缺口组合」里 3 个是跨源，只有 `emotion` 的 `em:push2ex/qdate`
是真的键对不上。跨源那 3 个要用 `input_evidence_ids` 表达（批 3）。

教训：**「机制描述正确」不等于「结论正确」** —— 少走了「这个 source 为什么是泛化的」
那一步。已更正 `TODO.md`，并写进教程第 50 章。

### 🔒 安全 · 生产库围栏：测试进程不许打开 `data/biga.db`

外部复核在清点派生证据时发现六类相加差 2 条，查出是
`task_id=BIGA-20260302-001`、`field=x`、`source=derived:x` 两行假证据 ——
**一次没设 `BIGA_DB_PATH` 的探针写进了真库**。

`tools/verify/probe.sh` 正是为第一次同类事故写的。**它存在，事故仍然重演。**

> 工具不等于守卫。`probe.sh` 让正确做法变得更短，但没让错误做法变得不可能。
> 只要错误路径仍然畅通，剩下的就全靠记性 —— 而记性正是追加式存储惩罚的东西。

⇒ 与本仓库那条禁网围栏同款：判据从文档搬进进程里。`conftest.py` 加 autouse
fixture，测试进程打开生产库直接抛错。**读也拦** —— 只读会让测试依赖本机恰好
有什么数据，与禁网同一个理由。真要生产数据就搬去 `tools/verify/`。

⚠️ 判据是**解析后的绝对路径**，不是文件名：sabotage 验证时改成按文件名判，
**22 个测试文件当场红**（全仓几十处 tmp 夹具都叫 `tmp_path / "biga.db"`）。
这条已写成测试，免得将来被「简化」掉。

🔴 **围栏一上线就抓到 17 条**：`test_news_scan.py` 的测试调 `is_trading_day()`
不传 `path`，一路走到 `DEFAULT_DB_PATH`，读的是**生产库的交易日历** ——
它们一直依赖这台机器上恰好有什么数据，而没有一行代码说过想要这个。

⇒ 补了第二层：autouse fixture 让每条测试默认拿到自己的 tmp 库路径。
两层分工是**默认安全**（fixture）+ **绕开就报错**（围栏）。只有围栏没有默认值，
等于让几十条测试各自记得设环境变量，又回到靠记性。

⚠️ 新围栏与既有的「`_store` 之外不许裸 `sqlite3`」守卫冲突 —— 两条都对：
走 `_store` 只能证明那一条路被拦，证明不了裸调用被拦。按仓库既有机制
（`# store-exempt:` 加理由）豁免，没有去改那条守卫。

新增探针 20 条（12 + 8），7 处 sabotage 全部验证会红。详见教程第 50 章。

### ✨ 新增 · 裁定 16 批 1：给证据一个身份（只加字段，不强制）

裁定 16 要求派生值声明 `input_evidence_ids`。动手前发现前置条件不存在：
**`Evidence` 没有 id**，引用无从谈起。所以本批只做身份，**不夹带任何强制**。

- `Evidence.evidence_id` —— 内容寻址（除自身外全部内容的 canonical JSON 的 sha256），
  `init=False`：**只由内容算出，构造方传不进来**。允许外部传 id 就等于允许传一个
  与内容不符的 id，而「引用对不上」正是这套东西要防的事。
  连带的好处：`from_dict` 读存量行时一律重算，库里若有与内容不符的 id，读出来就变对了。
- `Evidence.kind` —— `observed` / `derived` / `parameter` / `None`（未声明）。
  🔴 它的真正用途是**关掉 L-13**：批 2 要判断「这条该不该有输入」，
  今天唯一的判法是 `source.startswith("derived:")` —— 按字符串形状分类，
  而且正好落在一条新不变式的关键路径上。
- `Evidence.input_evidence_ids` —— 只接住不强制（capture 不 enforce，同批 J-I 的 `run_id`）。
- `SHA256_RE` 收敛成**一份**，`verdict_ref` 改为引用它。

为什么是内容寻址而不是分配器：risk 那 444 条派生值的输入在**上游的 verdict** 里，
序号类 id 必须先限定「属于哪份 verdict」才有意义，内容哈希本身就够；
且同一条证据在哪次运行算出来都是同一个 id（回放天然稳定）。

唯一一处现在就强制的：`kind="parameter"` 不许带 `input_evidence_ids`。
这不是对存量数据的要求，而是**拒绝自相矛盾的声明** —— 没有任何历史行会主动声明矛盾。

生产代码零改动通过全量（新字段全有默认值）。**新写的测试文件被守卫拦了两次**，
两次都拦对了，见「已知问题」下方的教训。

### 🔀 变更 · 裁定 16 的六类分布表：合计是 1719 不是 1721

外部复核指出六类相加差 2 条。核实结果：**那 2 条不是第七类，是探针行** ——
`verdict_id=264/265`、`task_id=BIGA-20260302-001`、`field=x`、`source=derived:x`，
同一秒写入，是测试里那个硬编码 TID。**一次没走 `BIGA_DB_PATH` 的探针直接写进了真库**，
与仓库记过的那次事故（`probe.sh` 就是为此而生）是同一个形状，**这是第二次**。
raw 层只追加，这 2 行删不掉 ⇒ 已在 `TODO.md` 标注「清点时要排除」，
免得批 2 去找一条不存在的口径。

### 🔀 变更 · 裁定 16：证据的类别与派生值的溯源（口径先定，实现分批）

评审 §16 后半（派生值声明 `input_evidence_ids` + `calc_version`）是 E 节**唯一**
剩下的条目。动手前先把口径定死，免得字段设计返工 —— 本条只改文档，不动代码。

⚠️ 顺带更正：批 R 的 TODO 条目写「E 节剩下六项」是**数错了**，把评审的章节号
当成了条目数。E 节 = §16–§20 共五项，四项已完成。

**量出来的事实**（生产库，2026-09-24）

- 派生证据占全部证据的 **55%（1721/3152）**
- 这 1721 条的 `raw_hash` 与 `evidence_set_id` **全部为空** —— 溯源链停在
  `derived:` 那个字符串上，走不下去
- 而且 **100%** 是「自己造一条同名证据、值必然相等」的形状 ⇒ 批 P 那条等值检查
  **对它们恒真**，它实际只守住了 45% 的直接字段
- `calc_version` **早就有了**，评审那句要求有一半本来就满足

**两条裁定**

1. **`Evidence` 显式声明类别**，不靠 `source` 前缀猜。今天要判断「该不该要求
   声明输入」只能 `source.startswith("derived:")` —— 那是 **L-13：按字符串形状
   分类**，而且正好在一条新不变式的关键路径上。三类：
   `observed`（指回 raw）/ `derived`（必须声明输入）/ `parameter`（两者都不要求）。
2. **派生值的输入引用粒度按字段定，不一刀切。** 理由是仓库已有的那条原则 ——
   **「不硬凑：凑出来的溯源比没有溯源更糟，它会让人以为查得到」**。
   `coverage_ratio` 的输入是「上游 verdict 存不存在」，硬塞一串 Evidence id 就是硬凑。

`parameter` 这一类与既有做法对齐：risk 的 `THRESHOLDS` 本来就不进 evidence、
只靠 `CALC_VERSION` 版本化。实测属于此类的只有 `news.window_min` 一个字段（47 条）
—— 原先估的「188 条无输入」是错的，另外 94 条 `derived:config` 其实有数据输入，
只是 source 标错了。

**记下两个会绊住实现的障碍**（写进 `TODO.md` 的批次输入，不留给下一个人重新发现）

- `observed ⇒ 必须有 raw_hash` **今天做不到**：直接证据缺 raw_hash 的比例
  09-20 100% → 09-24 **13.8%**，在收敛但**仍在产生** ⇒ 必须三段式，不能一步 enforce
- 今天的 19 条缺口只有 4 个组合，病因已定位：`market` 的 source 写成泛化的
  `sina:kline`（不带 symbol），而冻结表的键是 `sina:kline/sh000001`，
  前缀匹配要求 source 比键更长 ⇒ 泛化 source 匹配不上任何键，**静默返回 None**

口径落在 `CLAUDE.md` 裁定 16；逐字段的粒度表与三段式计划落在 `TODO.md`。
`architecture.md` 只加了一个指针并显式标注「尚未实现」—— 常青文档描述现状，
未实现的设计放进去就会变成长期挂着的「设计中」（Phase 1 §11 的教训）。
...
**E-17 · 递归冻结（`domain/_freeze.py`）**

契约类早就有 `@dataclass(frozen=True)` + `MappingProxyType(dict(result))`，
代码旁的注释还写明「frozen 挡不住对可变对象原地 mutate，必须两者都做」——
结论对，但**两件都只做在最外层**。嵌套的 list/dict 仍是外部那个对象：

```python
inner = [1, 2]
fb = FactBundle(..., result={"k": inner})
inner.append(777)          # → fb.result["k"] 变成 [1, 2, 777]
```

`Evidence.value` 连第一层都没包，外部还能事后加新键。

为什么这不是洁癖：**铁律在 `__post_init__` 校验，穿透发生在那之后** ——
落库的对象与被校验的对象不是同一份内容，且全程不报错。
实测生产库 3152 条证据里 **464 条（14.7%）的 `value` 是 dict/list**，
且这些同时也是 `result` 的顶层值 —— 两边**经常是同一个对象**，
于是批 P 刚加的「证据值必须与 result 对得上」可以被一次原地 mutate 同时满足。
一道交叉校验被「两个指针指同一处」架空，是最难看出来的那种绿。

🔴 **本批差点把批 P 的门重新推开**：冻结后 `_same_value` 拿到 `mappingproxy`，
`json.dumps` 不认 → 抛 `TypeError` → 落进「序列化不了就退回 `==`」的兜底 →
`1 == 1.0` 又成立。**全程不报错，批 P 自己的测试照样全绿**（它们比的是裸 dict）。
修法是先 `thaw` 再 `dumps`，并留了一条专门守这条回归的测试。
教训写进教程：改了被广泛消费的数据形状之后，要回头把**消费它的守卫**各跑一遍。

序列化边界 `thaw` 把 tuple 还原成 list ⇒ **卡片与落库逐字节不变**，
只有内存形态变了（8 处测试断言从 `[]` 改成 `()`）。这是能安全落地的前提。

**E-19 · `staleness_sec` → `source_lag_sec`，新增 `age_at(evaluated_at)`**

它量的是**取数滞后**（`retrieved_at − as_of`），不是年龄。而 `risk_check` 就是
把它当年龄报的，标签写着「最旧证据的年龄(秒)」——
一份三天前冻结的快照，只要当初抓取花了 2 秒，这个数就是 2。

docstring 其实把这个陷阱写得明明白白（还点名了 F15 共模误差），
误用仍然发生在同一个仓库里。**名字是唯一一处每次使用都会被读到的文档。**

⚠️ 诚实说清今天的严重性：实测 3152 条证据，取数滞后与「决策时真实年龄」
**最大只差 87 秒**（超 1 小时 0 条），且 `max_staleness_sec` **不在 `THRESHOLDS` 里、
不闸任何东西**。所以这不是正在发生的自动 fail-open，是一个**潜伏的错名字** ——
今天两个量碰巧接近，一旦冻结快照跨运行复用就分家。
把潜伏说成正在发生，下次真发生时就没人信了。

`age_at(evaluated_at)` 强制传基准时刻：读历史证据时「有多旧」只在「相对哪一刻」下
才有意义。`age_sec`（锚在 `now_cn()`）保留但注明禁止用于任何落库/上卡/回放的数字。
`risk` 现在两个数各报各的：`max_source_lag_sec` + `max_evidence_age_sec`。

⚠️ `news_scan` 的 result 里也有个 `staleness_sec`（意思是「距最新一条」，
量的是新闻源静不静）——**没动**，两处各加注释说明同名不同义，别顺手「统一」。

**E-20 · 采集出处三件事**

- **20.1 时刻记早了**：`retrieved = now_cn()` 写在 `_fetch(...)` **之前** ——
  记的是「我打算去抓」。抓取耗时全被算进「数据有多新」，偏差**单向**（只高估）。
- **20.2 来源写死了**：`source = f"sina:kline/{symbol}"` 硬编码，而 fetcher 是
  **可注入的** ⇒ 换任何 provider 进去，raw 层照样记「sina」。
  可注入的依赖 + 硬编码的标识 = 一条必然说谎的记录。现由 `IndexDaily` 自己声明
  `provider_id` / `adapter_version` / `source`，并进 manifest。
- **20.3 指纹没人验**：`content_sha256` 只在写入时算过一次，此后从未被检验过。
  一个从不被检验的指纹，和没有指纹的区别只在于它让人放心。

🔴 20.3 差点写成一个 R-3 违规：第一反应是「85.4% 的历史行 `raw_text` 是 NULL，
验不了就跳过」——跳过就是静默 fail-open。**先去量，而不是先设计降级**：
实测 376 行 raw，55 行走 `raw_text_sha256`、321 行走 v13 之前的 `payload_sha256`，
**两边各自全部一致、0 例外**。于是根本不需要「验不了」那一档，直接 fail-closed。
在为「验不了的那部分」设计降级之前，先确认它真的验不了。

**顺带**：`risk` 的 `_EXPECTED_FIELDS` 10 → 11。加字段忘了改它会让
`data_completeness` 算出 1.1 被契约当场拒掉 —— 刺耳但正确，截断到 1.0 会把
口径变更悄悄抹平。已在常量旁记下这次实例。
...
**E-16 · `result` 的值必须与它自己的证据一致（评审 §16）**

在这条之前契约只查「`result` 的键有没有同名 Evidence」，不查两边的**值**。
评审举的这个例子能被原样接受、落库、上卡：
...
一个卖点是「证据可追溯」的系统，最不能有的就是它：卡面那个数与它声称的证据
对不上，而每一道既有检查都通过 —— R-3 的形状，看起来被验过，其实没有。

判据取 **canonical JSON**，与回放比对同口径（`1` 与 `1.0` 算不同）——两处口径不一致
会出现「契约说相等、回放说不等」这种最难查的矛盾。同名证据不止一条时判据是
「**至少有一条**对得上」：多来源合成时逐条判等无从谈起，但也不能因此放行，
否则加一条装饰性证据就能绕过整条检查。

⚠️ 实测生产库 411 个 result 字段，canonical JSON **全部相等**，0 个例外 ——
这条不误伤任何存量数据。

🔴 第一版把「相等」和「合法」搞混了：用 `allow_nan=False` 序列化，NaN 失败后退回
`==`，而 `nan != nan` ⇒ 两边**本来就相同**的 NaN 被判成对不上，抢在真正该报错的
那道（写边界的严格 JSON）前面报了个不相干的错，实测打红 `TestP4StrictJSON` 六条。
改成 `allow_nan=True`：这里只问「一不一样」，合不合法是写边界的事。

**E-18 · 缺席 agent 的登记从「比条数」升级成「对得上号」（评审 §18）**

🔴 **先更正我自己上一轮报错的一条结论。** 我当时说「五个 agent 缺席可能在卡上只
显示成一条」—— **不成立**，端到端复现证明卡上五条一条不少：`card_ops.synthesize()`
按「代码 + 文本」复合键去重，文本不同就都留着。我把 `MissingItem.__eq__` 只看
`.code` 这个**局部属性**当成了端到端的数据丢失，没有走完全链路就下了结论。
...
`_check_roster` 只能比条数，是因为按文本去猜「这条 missing 说的是不是那个缺席的
agent」属于 L-13（按字符串形状分类）。⇒ 把 agent 名字放进代码
（`supervisor.<agent>.<reason>`，新增 `absent_agent_code` / `absent_agent_of` /
`absent_agent_missing`，生产与测试共用一份实现），对应关系变成**结构化**的，
不需要猜任何文本 —— 这是能收紧那道检查的前提，不是放弃了 L-13 那条纪律。

判据现在是：**每个缺席的 agent 都要有一条解得出它自己名字的缺失项**。
对应成立 ⇒ 条数必然够，所以旧那条计数判据被**取代**而不是并存。
三段式照旧：新卡拒 / 旧卡（`supervisor.agent_offline` 解不出 agent）可读并记
`roster_warning` / 落库永远拒。

⚠️ 连带影响：十一个测试文件里约 25 处 `missing=[f"占位{i}——本文件不测 roster" …]`
——那是在旧的**计数**判据下凑数用的，正是新判据要拦的东西。统一改成
`tests/_roster.py::absent_registrations()`，登记的就是真的缺席的那几个。
`synthesize.py --extra-missing` 的帮助文本同步更新。

**这一批没做的九项，以及为什么**

* **E-16 的后半（派生值显式声明 `input_evidence_ids` + `calc_version`）** ——
  实测：411 条证据里 **244 条（59%）** 的 `source` 以 `derived:` 开头，涉及六个
  skill、39 种 `(agent, field)`。每种都要说清「这个数是从哪几条原始证据算出来的」，
  是独立的一块领域工作，不是契约层改一改的事。本批那条等值检查**拦不到**它
  （派生值的 result 与它自造的同名 Evidence 天然相等）—— 这一点在契约的注释里写明了，
  免得被读成「已经覆盖」。
* **E-17 deep_freeze / E-19 `source_lag_sec` + `age_at(evaluated_at)` /
  E-20 Snapshot 元数据四项** —— 都已实测确认成立，但各自有独立的 blast radius
  （E-19 会动 risk 的判据口径），留给下一批。

### 🐛 修复 · 上一条修复自己漏了一处、且守卫本身不够硬——四轮复核

对"消除动态同名模块 monkeypatch 错位"这条修复本身做了第四轮复核，发现两处
🟡（守卫效力问题，不是生产代码正确性问题）：

**新加的运行时身份断言其实依赖收集顺序**：`TestModuleIdentityAcrossTestFiles`
的文档字符串写着"不依赖收集顺序"——不成立，已被实测证伪。把某处幂等检查
完全删掉，默认字母序全量 `pytest -q` **一条不红**：12 处 `_load` 里有 6 处
（`test_decision_card`/`test_emotion_calc`/`test_facts_split{,_e2,_e3}`/
`test_market_calc`）排在 `test_orchestrator.py` 前面收集，这类顺序下
`orchestrator.py` 自己的 `import` 反而会"捡漏"到违规文件已经登记的对象，
两者天然一致，断言通过——运行时身份断言能不能抓到问题，本身就取决于
"覆写方是不是恰好排在检查方之后"。

修法：新增 `tests/test_module_load_idempotent.py`，源码级 AST 扫描（同
`test_no_raw_sqlite.py`/`test_contract_single_impl.py` 的风格，不靠运行时
表现）——任何 `sys.modules[...] = ...` 赋值，所在的函数（或模块顶层）内
必须能找到一个 `... in sys.modules` 的判断，否则判违规。这条不依赖任何
收集顺序：判据是源码文本结构，不是运行时对象是否分裂。运行时身份断言
保留作为第二层信号，文档字符串已改正，不再声称"不依赖顺序"。

**同一次提交里，改过的那个文件又漏了一处**：`tests/test_risk_check.py`
这次刚好补上了 `risk_check` 那处的幂等检查，但同一个文件里另一处加载
`latency_report`（`TestStageTopology._lr()`）用的是一模一样的无条件覆写
逻辑，这次改动**没有扫到自己身上**——"批 J-II 只改了当时正在改的那一个
文件"这个叙事，在治它的这次提交里又发生了一次，只是这次载体换成了
"同一个文件里的旁边一处"。目前无实证的生产/跨文件碰撞后果（`test_verify_
exit_codes.py` 全程用自己那份 `lr` 引用打 monkeypatch，暂未真的分裂过），
但形状与已确认会红的那几处完全一致，一并补上幂等检查。

这两条都是被四轮对抗性复核用真实实验（不是读代码）抓到的：先验证"删掉某处
检查后默认全量是否真的会红"，再自己写扫描去抓，第一遍就抓到了这处漏网——
比"我觉得应该扫过了"更可信的是"跑一遍扫描器，看它自己报什么"。
...
`decision_runs.created_at`（run **起跑**的时刻）与当前时刻的差判断是否
stale，不是最后一次真正推进的时刻——判据因此其实是"起跑太久"，不是
"没推进太久"。真机复现：一个起跑很久、但刚刚才正常推进到 `STAGE1_RUNNING`
的健康 run 会被误判成 stale 并收成 TIMEOUT，编排器接着想推进时被 CAS 正确
拒绝（不会静默覆盖成一张坏卡），但一次正常运行就此被打断。默认配置下走
`bin/biga-card` 撞不到（外层硬超时 840s < 阈值 1200s，会先被外层杀掉），但
把 `BIGA_CARD_DEADLINE_SEC` 调过 1140s，或直接跑 `orchestrator.py`（没有
外层 timeout——正是 D-3 要防的场景），就会撞上。改用 `run_events` 最新一条
的 `at`（最后一次真正推进的时刻）。新增两条测试：起跑很久但刚推进的不该
被误杀、真正卡住不动的仍要能被收敛（确认修法没有连带失效）。
...
- **最严重的一处**：「手机上能看」小节还在警告"别用自然语言在飞书里要卡"、
  "编排现在只有一份实现...这条路径的实测收口还没做"——这是批 G-II（Inbound
  Trigger）**之前**的旧世界描述。批 G-II 早就把 `/card` 命令这条路径改成
  结构化 trigger、`main` 完全不参与编排，且已经 P6 live 端到端确认完成。
  旧措辞会让读者误以为这个功能不安全/没测过，而实际情况正好反过来。改成
  区分两条路径：`/card` 命令 ✅ 已验证安全，裸自然语言仍然不安全（`main`
  认不出显式技能引用，还是会自己临场编排）。
- 「确定性编排升级」的批次表格还停在 E-III/J/F–L 都没做完的状态，实际上
  A 到 L、H-I、H-II 全部已经独立复核通过合并——重写成真实进度，同时如实
  标注 H-III 是主动裁定留白（不是"还没排上"），跨包引用清理是唯一真实
  待办。
- 「收盘后日线几点发布」那段还写着"正在实测，测出来之前先用这条命令自己
  查"——这个问题 2026-09-21 就有了确定答案（15:32:47–15:37:50），直接给出
  结论，命令保留作为"想自己确认就跑这条"的备选。
- 目录结构图完全没反映批 H-I/H-II 的包重组（`src/easyup_biga/` 五个真实
  实现 + `skills/` 下五个兼容薄壳），补上；顺带把"四个数据源"更新成批 L
  之后的真实数量（五个，加了深交所官方交易日历）。
- 开发教程表格只列到第 10 章，没有任何"还有更多"的引导，容易让人以为
  教程到此为止——补一句指向完整索引（现在 39 章）。
- 两处提到具体分支名（`phase2` 已并入 / `orchestration` 分支）的标题，
  改成不依赖分支名的状态描述——分支这类实现细节对读者价值不大，还特别
  容易在下次合并后又变成新的过时点。

**没有改的**：1334 条测试徽章是准的——`test_文档里的测试条数与实测一致`
自己的判据会排除掉本地未跟踪的参考材料（`docs/external/` 下六份），手动
`pytest --collect-only` 数出来的数字会被这些文件抬高，不能拿来对着改。

### 🔧 变更 · `images/` 里的文件名统一命名规则

`bigA01.png`（透明底白字变体）跟其余四张 `LOGO_BigA0N.png` 命名不成一套——
缺 `LOGO_` 前缀、开头大小写不一致，编号还跟 01 号重叠，容易让人误以为
是跟 `LOGO_BigA01` 平行的另一份设计，而它其实是 01 号的变体（`f202049`
那次加它时讲得很清楚：透明底白字与黑字白底互补，不是替代关系，只是没有
把这层关系体现进文件名）。改名为 `LOGO_BigA01_Transparent.png`，README
「品牌素材」清单同步更新（把互补而非替代这条说明也补进条目本身，之前只
写在 commit message 里）。历史 CHANGELOG 条目里的旧文件名不回改——那是
对"当时"的真实记录。
