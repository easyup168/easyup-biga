...
| 轮次 | 耗时 | 输出 tok | 成本 |
|---|---|---|---|
| main（spawn + yield） | 21.1s | 1,506 | $0.071 |
| emotion（真正干活的） | 47.8s | 4,152 | $0.095 |
| **main（合成）** | **136.2s** | **9,325** | **$0.453** |

最大的一块是 **Supervisor 的合成轮**，不是采数据的 emotion。
输出 9,325 token 说明绝大部分是 `thinking=high` 的思考过程。
单次决策约 **$0.62**。

下一步：thinking 三档受控实验，同时测延迟**与结论是否改变**。
