...
**⚠️ schema 版本号撞车提醒**：本批开工时（HEAD=1824361）v11 是下一个空号；但并行的
**批 F**（当时未合并、另一棵工作树）也占用 v11。两批并进同一条主线时 v11 会撞 —— 套路
同 J-I/J-II 处理 v9/v10：**谁先合并谁占 v11，后合并的那批开工前先 `git log` 确认对方
是否已落地、占下一个空号**（重编号只改 `MIGRATIONS` 尾部字面量，迁移体不动）。`schema.py`
`_V11` 注释里留了同一条记号。
