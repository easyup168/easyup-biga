...
### 文档 · 把 `README` / `CLAUDE.md` 的自我描述改成 Phase 3 的真实状态

合并到 `main` 之前必须做这件事 —— `main` 是 Public 仓库访客唯一会看的分支，
而它上面的自我描述会**以本页为准**。裁定 14 守的就是这条。

改了什么：

| 位置 | 原来（错在哪） | 现在 |
|---|---|---|
| Status 徽章 | `phase 2 code closed`（**绿色**）| `phase 3 code closed · exit 0/3`（**琥珀**）——代码写完不等于验收通过 |
| 实现状态表 | `Dataset / Provider / Pipeline Registry ⬜ 后续` | ✅ 12 dataset / 17 provider / 20 条绑定边 / 8 条 FALLBACK。**这条是反方向的过期：已经交付了却还写着「后续」** |
| tag 现状表 | 「`v0.5.0` 是当前 `main`，本页的数字描述的就是它」 | 本页数字描述的是 `phase3` 的 HEAD。**Phase 3 没有 tag** —— 出口条件 0/3，还不到打 tag 的时候 |
| `CLAUDE.md` schema | `v22`，十一张表 | **`v27`，二十张表**（Phase 3 从 v23 起） |

新增一节「Phase 3 出口条件：代码面已过，证据 0/3」，直接写出闸门输出：
十项代码面检查全绿、EOD **0/5**、演练 **4/6**。

### ⏩ 更正 · `CLAUDE.md` 里那句「已解决」是假的
...
**没解决。** 实测历史 12 次出卡，**9 次**仍在报「上游交易日不一致」。
真正的两个原因 2026-09-26 才查出来，都与日历无关（见 [0.9.18]）。

> 一条写着「已解决」的状态，比写着「未解决」更危险 —— **没人会回头核对它。**

### 合并纪律

往 `main` 的合并**一律走 PR**，由人点 merge。
feature 分支负责把活干完（代码 + 文档 + CHANGELOG + 版本号 + 全量测试 +
`audit_public.sh` + 条数同步），PR 只等人看。

---

## [0.9.21] - 2026-09-26

### 新增 · 最后一个单点源配上备胎，而且**换了一家**

`cn.market.limit_pool` 是当时唯一没有备胎的 dataset。2026-09-26 东财
`push2` 系四台主机同时 502 ⇒ emotion **整组 UNKNOWN**，卡上少了一整个维度。

东财自家还有一条路（付费 AI 接口 `mkapi2.dfcfs.com`，实测当天可用，
push2 全挂时它三次调用全通）—— 但它**和主源同属一家**，
厂商级故障会一起挂。**真正的备胎必须换一家。**

⇒ 新增 `sina_limit_pool`：从 `sina_eod` 的全市场快照**自算**三个股池。
与 `sina_breadth` 同一个模式（同一份快照、不另写分页）。
...
差了四倍多。「曾封板、后打开」需要分时/逐笔，单日 OHLC 里没有这个信息。

> 它不会报错 —— 只会给出一个看起来精确的、**错四倍**的百分比，
> 然后印上卡面。

⇒ 不给。适配器只交三个计数。

### 🔴 `PoolResult.row_fields` —— 「这个源的明细里真正有哪些字段」

备用源给的明细比主源少，而**少掉的那些都有默认值，且默认值指向最危险的方向**：

| 缺什么 | 不声明的话会算出什么 |
|---|---|
| `lbc` | `int(r.get("lbc") or 1)` ⇒ 每只都算首板 ⇒ `max_streak=1`、`streak_ladder={1:N}` —— **宣称今天一个连板都没有** |
| `zbc` | `int(r.get("zbc") or 0)==0` 恒真 ⇒ `seal_never_broken_rate=1.0` —— **宣称全都没炸过板** |
| 整个 `rows` | 上面两条同时发生，外加 `seal_never_broken_rate=0.0` |

**三种错法都不报错。** 而且 `rows=[]` 本身是**合法状态** ——
东财盘前实测就是 `tc=0` + `pool=[]`，那是「今天真的 0 家」。

⇒ 「有没有这个字段」不能从数据里推，只能由适配器**说出来**。
落库（schema v2 加 `row_fields_json`）、读回来、逐项守卫。

emotion 现在按字段声明逐条判：给不了就进 `missing[]`
（`emotion.streak.unavailable` / `emotion.seal_rate.unavailable`），
而**三个计数照常产出** —— 那比「全部无法判定」有用得多。

### 修复 · 股池 Evidence 的 `source` 写死在 8 处（第三次了）

`em:push2ex/{pool}` 的字面量。板块（v0.9.9）、涨跌家数（v0.9.15）
都踩过，这是第三处。新增 `_pool_source()`，`read_pool` 交出实际供数方。

⚠️ 备用源是**自算**的不是报数 ⇒ source 形状也不一样（`sina:hs_a/{pool}`）。

### 修复 · 冻结链里另一处 `lbc` 默认值

`cn.market.emotion_close` 的 `max_consecutive_limit` 也是
`int(row.get("lbc") or 1)` 算的 —— 备用源供数时会得到「今天最高 1 板」。
源没声明这个字段就交 `None`（R-3）。

### 修复 · 我差点让备用源把全市场拉三遍

`[_sina_all_pools(d)[n] for n in names]` —— 每次迭代都重新抓一遍
（约 5 MB × 3 / 24 秒），而且三个池在时间上还会不一致。
改成先抓一次再取。**这行注释是它的墓志铭。**

### 涨跌停判定的两处实测细节

- **阈值按板块**：科创/创业 20%、北交所 30%、其余（含 ST）10%。
  认不出板块的行**跳过，不默认 10%** —— 默认会把 20% 涨停漏掉，而且不报错
- **容差用 9.9 / 19.9 / 29.9，不用 10 / 20 / 30**：涨停价按昨收
  四舍五入到分算，实际涨幅经常不是整数（昨收 8.13 ⇒ 涨停 8.94 ⇒ +9.963%）
- **ST 现在也是 10%**（2026-07-06 新规，主板 ST 由 5% 提至 10%）⇒
  不需要按名字判 ST 了。**这条将来若再改，是这个适配器要跟的**

### 已知问题

- 三个源的口径互不相同，这是**性质差异不是误差**：
  东财 52/13/10、新浪自算 54/15/14、东财妙想 53/16。
  备用源只在降级时供数，且 Evidence 的 `source` 会如实写出是谁
...
写 `docs/tutorial/` 用的**一手材料** —— 真实命令、真实输出、真实报错、
以及「我当时是怎么想的、哪一步想错了」。

🔴 **为什么需要它**：教程的价值在真实的建造过程（裁定 10），
而那些细节**在当下是活的，一周后回忆就只剩「装上了、能跑」**。
以前没有地方放它们，于是每次都是事后重构记忆。

**为什么在仓库外**：与 `local-skills/` 同一条理由 ——
素材常带本机情况（真实路径、账号、同机已有实例的细节），
而那正是「公开仓库纪律」要挡的东西。

不用 `.gitignore`：那是一条**需要人记得**的规则
（`git add -f` 绕得过、整理时会没、脚手架复制不跟着走）。
放仓库外，`git add` 根本够不着。

⇒ 仓库里只多一段**指针与机制**（`CLAUDE.md` 的「教程纪律」下），
内容留在外面。与凭据那条纪律同一个形状。

⚠️ 从素材搬进教程时要**过一遍公开仓库纪律**：抽掉本机路径、账号、
同机已有实例的可识别细节，保留可迁移的工程结论。

第一份素材是「Phase 3 数据平台在一次真实故障里到底起了什么作用」——
记的是 `BIGA-20260926-003` 那张 WAIT 卡的完整排查过程
（五条 SQL、崩溃 agent 的原始 transcript、以及被错误信息带偏的那一轮）。

---

## [0.9.18] - 2026-09-26

### 🔴 risk 长期「无法判定」，两个原因都不是风险本身

历史 12 次出卡里 **9 次**报「上游交易日不一致」，而判断表写死
「交易日不一致 ⇒ 无法判定」。查下来是两种完全不同的形状：

#### 原因一 · `news` 用 `trade_date` 装了**不是交易日**的东西

7x24 是**连续事件流**，它没有「交易日」这个概念。那个字段装的是
「最新一条快讯发生在哪天」（标签一直写着「最新一条所属日期」），
和 market/technical/sector/emotion 的「这批行情属于哪个交易日」
是**两件事**，只是碰巧都长成 `YYYYMMDD`。

而 risk 把**所有**上游的 `trade_date` 放进同一个一致性判据 ⇒
**每一个非交易日**（以及每天日线更新之前）都必然报假冲突，
而 news 说 09-26、行情说 09-24 **两边都是对的**。

⇒ 改名 `newest_flash_date`。改完当天 20:22 的真实出卡已经是一致的。

> 同名不同义**不会报错**。它只会让一个判据长期报一个假结论，
> 而读的人以为那是真的。

#### 原因二 · 盘中「日线锚定昨天、实时锚定今天」是**结构性**的

实测 `BIGA-20260924-006`（盘中 11:20）：

    market / technical / sector = 20260923   ← 日线：今天还没收盘
    emotion                     = 20260924   ← 股池：盘中就是今天的

**两边都对。** 每个盘中运行都会出现。把它和「跨了三天」压成同一个
`False` 交给 agent，而判断表说「不一致 ⇒ 无法判定」——
于是 **risk 在盘中几乎永远无法判定**。

⇒ **跨度是事实，「算不算矛盾」是判断**（铁律 4）。skill 现在交出：

| 字段 | 作用 |
|---|---|
| `trade_date_span_days` | 最大跨度（天）。0 / 1 / ≥2 是三种完全不同的处境 |
| `upstream_trade_date_by_agent` | 逐 agent 明细 —— 只给一个 `False`，agent 分不清「谁混进来了」和「日线没收盘」 |
| `trade_date_reporters` | **谁参与了这条判据**。「3 个一致」和「只有 1 个报了」原来在卡上长得一模一样，而后者其实什么都没核对成 |

`agents/risk/AGENTS.md` 的判断表相应拆成三行（跨度 1 天且较新那天是今天 ⇒
盘中正常，不要据此无法判定；已收盘却差 1 天 ⇒ 警示并点名；≥2 天 ⇒ 真矛盾）。

⚠️ 缺失项文案只**提示**哪种形状通常正常，**不替 agent 下结论** ——
末尾一句「这个判断留给你」由测试钉住。

### 新增守卫 · `test_trade_date_field_semantics.py`

钉「**名字 ⇒ 标签**」的绑定：叫 `trade_date` 的，标签只能是「交易日」。

🔴 **为什么不是列一份 agent 白名单**：白名单挡不住「新 agent 又用这个名字
装别的东西」，而标签不一致挡得住。

还有反向的一半：**news 不得产出 `trade_date`**。
没有这条的话，把标签改成「交易日」就能骗过上面那条 ——
探针验过这个绕法，第二条确实拦住了。

### 已知问题

- 这两处只改了**判据与文案**。risk 在盘中到底给不给方向，
  仍取决于 agent 读完新判断表怎么判 —— 下一次真实盘中出卡才知道

---

## [0.9.17] - 2026-09-26

### 裁定 · 板块以东财为主源，备用源必须**声明口径**

A 股板块数据是个课题 —— **行业、概念各家的分类都不一样**。实测：

| kind | 东财 | 新浪 |
|---|---|---|
| `industry` | 496 个申万式细分（`BK1456 其他家电Ⅲ` / `棉纺`） | **84 个证监会门类**（`hangye_ZA02 林业` / `畜牧业`） |
| `concept` | 500 个概念 | 175 个概念（`gn_hwqc 华为汽车`） |

⇒ 备用源**不是「同一个榜的另一个来源」，是另一个榜**。
降级那天卡上会说「领涨板块 = 林业」—— 那是证监会门类里的一个大类，
对短线基本无用，而它和主源那个「棉纺 +8%」根本不是一回事。

**为什么仍以东财为主**：各家分类不一致时，**跨天可比性比可用性更重要**。
昨天的第一名和今天的第一名不在同一个榜上，趋势就无从谈起。

**为什么备用源仍然保留**：有总比没有强，但必须**声明**。

### 新增 · 板块分类体系作为**事实**上卡

`sector-calc` 现在逐 kind 产出 `industry_taxonomy` / `concept_taxonomy`
（如 `em-sw-细分(496)` / `csrc-门类(84)`），并在与主源不同时进 `missing[]`。

🔴 **进 `missing[]` 而不是 `warnings[]`**：警告会被读成「注意一下」，
而这里的事实是**这个榜和你以为的那个榜不是同一个，不能拿来比**。

🔴 **口径本身也要上卡**，不能只发 missing —— 不上卡的话，
「领涨板块 = 林业」旁边没有任何东西说明它出自 84 个证监会门类，
而那正是它看起来荒谬的原因。

⚠️ 直连路径原本把 `served_by` 记成 `None` ⇒ **分类体系那条事实产不出来**，
而直连时口径其实是已知的。「不知道」和「知道但没填」在卡上长得一样。
改成显式记 `eastmoney`。

### 修复 · `_EXPECTED_FIELDS` 与新字段同步

`data_completeness` 的分母。加了两个字段没同步 ⇒ 算出 1.22 ⇒
契约当场拒绝。**这次是响亮的**；反方向（分母调大、分子没跟上）会被
静默稀释，更糟。已在常量旁写明「加字段必须同步改它」。

### 文档

- `docs/design/data-sources.md` 新增 **§3.13**，记下这条裁定与实测口径表
- §3.10 的标题（「口径少两个字段」）**本身低估了差异**，加了指向 §3.13 的更正
- 待办新增两条：①**个股所属板块没有本地库**（成分会变动；同机已有实例每周六
  下载一次落本地 DB，我们迟早需要同一份）②**东财是四个 dataset 的共同风控面**，
  一次封禁四个一起降级 —— 限流虽已补齐，同源集中本身是结构性风险

### 已知问题

- `docs/design/data-sources.md` 的 §3 小节编号**本来就是乱的**
  （3.5 在 3.3 之前、有两级标题混用），本次没有一并整理 ——
  改编号会让所有历史引用失效，值得单独一次做

---

## [0.9.16] - 2026-09-26

### 🔴 不带日期的端点，`as_of` 两个方向都标错过

涨跌家数与板块榜这两个端点**不返回交易日**。它们该署哪个时刻，
本仓库先后踩过**方向相反**的两次：

| | 做法 | 后果 |
|---|---|---|
| F4 之前 | 用**日线的交易日** | 周一 12:41 午休实测：取回的是今天此刻的 4385/1100/145，卡面写 `as_of 09-18 15:00` —— **一个今天的数，挂着上周五的时间戳** |
| F4 之后 | 用**取回时刻** | 周六 18:11 实测：端点给的其实是 **09-24 收盘**的 1120/4305/137（它自己不会说），卡面写 `as_of 09-26 18:11` —— **一个上周四的数，挂着周六的时间戳** |

> 两次都不是取错了数，是**给对的数配了错的时刻**。而它不报错。

第二次的代价是可量化的：risk 因此判「上游报告了不同的交易日
（market/technical=20260924，news=20260926），不能当作同一天的事实一起审」，
整张卡降级成 `WAIT` —— **而那个「不一致」有一半是我们自己标出来的**。

⇒ 新增 `tradetime.as_of_for_undated_snapshot()`，**三态、靠日历不靠推算**：

| 此刻 | 端点给的是什么 | `as_of` |
|---|---|---|
| 交易日 & 收盘前（含午休） | 今天的盘中快照 | 取回时刻 + 盘中警告 |
| 交易日 & 收盘后 | 今天的收盘 | 今天 15:00 |
| **非交易日** | **最近交易日的收盘**（不再变化） | **那天 15:00** + 警告 |

`market` 与 `sector` **共用同一份实现** —— 各写一份就是 L-3，
而那份差异的表现正是「两个 agent 报了不同的交易日」，不报错。

### 新增 · `persistence.latest_trading_day()`

「`day` 之前最近的一个交易日」，查 `fact_trading_calendar`。

🔴 **只在已抓到的日历范围内回溯，不做任何推算。**
「往前找到第一个非周末」那种推算会在长假里给出错误答案，
而**错误答案和正确答案长得一模一样**。查不到返回 `None`（R-3），
调用方退回取回时刻**并把这件事说出来**。
...
两句都在描述一个**已经不存在的行为**（F4 之后 as_of 就不按日线算了），
现在更是由日历判。留着比删掉更糟：它们让读的人相信一个错误的机制。

⚠️ 对应的两条测试原来钉的是**文案**（`"不返回交易日字段" in w`）。
改成钉**意图**（必须有一条警告提到 `as_of`）——
钉文案会在每次措辞微调时红一次，而那种红不含信息。

### 新增守卫 · `test_undated_snapshot_as_of.py`

**两次事故各钉一条，方向相反的两端都钉住。**
只钉一端的话，修它的人会顺手把另一端改回去（历史上已经发生过一次）。

还钉了一条：`market` 与 `sector` import 的必须是**同一个函数对象**。

### 已知问题

- 板块的「行业」降级源**口径不对**：新浪给的是 84 个证监会门类
  （农业/林业/畜牧业），东财是 496 个申万式细分（棉纺/焦炭Ⅱ）——
  **不是同一个事实的降级，是另一套体系**。
  降级那天卡上会说「领涨板块 = 林业」，对短线基本无用。
  概念板块两家可比（新浪 175 / 东财 500），行业不可比。下一版处理

---

## [0.9.15] - 2026-09-26

> 起因是一次真实出卡（`BIGA-20260926-003`）：`WAIT`、7 项缺失、
> `已建成 roster 缺席：sector`。顺着查下去挖出**四个独立缺陷**，
> 其中两个是我自己前几次改动留下的。

### 🔴 一、sector 整支缺席 —— skill 在写任何证据之前就崩了

`sector_calc.py` 的 `board_counts` 那一行写死了两个字面量：

```python
add_live("board_counts", counts, "各榜板块数", "em:clist", kind="derived",
         origins=raw_origins(c.hashes.get(f"em:clist/{k}") for k in (...)))
```

备用源供数时 `c.hashes` 的键是 `sina:bankuai/*` ⇒ **两处都查不到** ⇒
`raw_hash` 与 `derived_from` 同时为空 ⇒ 契约铁律 `ValueError` ⇒
脚本崩溃 ⇒ **卡上少了一整个 agent**。

而冻结层其实**成功**冻到了板块快照 —— 数据在，是 skill 自己没用上。

> 它**只在降级那天炸**。主源正常时两边字面量碰巧相等，
> 所以既有测试和此前每一次真实出卡都是绿的。

🔴 **这是 [0.9.9] 那次改动的回归。** 当时引入 `_board_source()` 就是为了
「source 不许说谎」，却漏了这一处 `origins=` 的查表键 ——
**判据跟着症状走（「板块的 source 对不对」），就只会修到症状那一个。**

### 🔴 二、涨跌家数的 source 在说谎 —— 同一个洞，上次只修了板块

`market_calc.py` 把 `"em:push2delay/ulist.np"` 写死在 **7 处**，
而 `read_breadth()` 压根不返回供数方（`read_boards` 有、它没有）。

当天东财 push2 系整组 502、实际由 `sina_breadth` 供数 ⇒
卡上**五条涨跌家数证据全部指着一个没供过数的端点**。

⇒ `read_breadth` 改为交出 `(结果, raw_hash, **实际供数方**)`，
`market_calc` 新增 `_breadth_source()`。
⚠️ 备用源是**自算**的（从全市场快照数出来）不是报数 ⇒
它的 source 形状也不一样（`sina:hs_a/breadth`），不是换个前缀就行。

### 🔴 三、跌停池**从来就取不到** —— 排序字段填的是另一个池的

三个股池共用了写死的 `sort=fbt:asc`（首次封板时间）—— 那是**涨停池**的字段。
2026-09-26 实测（同一天、同一个 date）：

| 池 | `fbt:asc` | `zdp:asc` | `fund:asc` |
|---|---|---|---|
...
⇒ 改用 `zdp:asc`（唯一三个池都支持的字段）。修完 emotion 立刻
`PASS`：涨停 52 / 跌停 13 / 炸板 10 / 情绪分 54.03。

**顺带修了那条把人带偏的错误信息**：原文一口咬定「分页可能截断」，
于是排查方向是 page_size 和翻页，而真相是一个 query 参数填错了池。
现在按「实测过的可能性」排序 —— 0 行时先说排序字段。

### 🔴 四、限流器接了 1/5 —— 而这正是东财整组挂掉的原因

`http.throttle()` 早就存在，docstring 把危害与实测都写清楚了：

> 实测连打十来个请求之后，连**已知可用**的兄弟端点也一起返回 502，
> 封的是来源而不是某个接口。

**然后它被接进了五个东财取数点里的一个。** 没接的四处恰恰是请求量最大的：
板块榜分页、EOD 分页（最多 80 页）、股池、涨跌家数。

当天实测：`push2` / `push2delay` / `push2his` / `82.push2` **四台同时**
502 / RemoteDisconnected，而请求量小得多的 `push2ex` 照常 200 ——
这个分布更像**按来源限流**，不像四台机器一起宕机。

参考实现（公开项目 `a-stock-data`）明确记着东财的风控阈值量级：
每秒 >5 次 / 单 IP 并发 ≥10 / 1 分钟 ≥200 次 / 5 分钟 ≥300 次 → 临时封 IP。

⇒ 四处全部接上 `throttle("eastmoney")`，**换主机重试那次也要等**
（封的是来源，换个 host 继续打只会把封禁坐实）。
...
| 守卫 | 挡什么 |
|---|---|
| `test_eastmoney_throttle.py` | AST 扫 `providers/eastmoney*.py`，每个发请求的函数都必须调 `throttle`。模块清单用 glob 不手写 —— 手写会在下一个 `eastmoney_*.py` 出现时静默漏掉 |
| `TestFallbackServedSnapshot`（sector） | 用**备用源供数**的冻结快照跑完整个 skill：不许崩、`derived_from` 不许空、source 不许指着没供过数的源、两个榜分属不同 provider 时要如实说「混合」 |

两处都做过探针：撤回修复后红、还原后绿。

⚠️ 限流守卫查的是「同一个函数里有没有 throttle」，**不保证顺序正确** ——
它挡的是「整个忘了」，不是「写错了位置」。这个边界写在模块头里，
免得读的人以为它管得比实际更宽。

### 已知问题

- **东财 push2 系仍然不可用**（截至 2026-09-26 19:4x 复测仍 502）。
  限流已补上，但封禁的冷却时间未知 —— 不能据此断定限流「修好了它」
- `cn.market.limit_pool` 仍无备用源，且 `read_pool` / `read_index_quote`
  也还没交出供数方（它们现在是单源 ⇒ 碰巧不会说谎，
  **加备用源那天就会** —— 与本次 breadth 同一个形状）

---

## [0.9.14] - 2026-09-26

### 新增 · 财联社电报接上来，成为 `cn.news.flash` 的主源

`cn.news.flash` 是最后两个**单点源**之一（另一个是 `cn.market.limit_pool`）。
现在的链是 **财联社 `v1/roll`（PRIMARY）→ 新浪 7×24（FALLBACK）**。

换主源换来的是 **`level` 这个真信号**（源侧重要性档位 A/B/C），
代价是条数 —— 两者同窗口实测差 4 倍。**两件都写在这里，不只写前者。**

### 🔴 一条 ❌ 把一个可用的源挡了整整一个 Phase

`sina_news` 的模块头里写着：

> 财联社电报 ❌ 404，接口已变

**那是错的。** 死掉的是旧的 `nodeapi` 系接口，不是财联社。官方
...
> 当时真正观察到的是「某个 URL 今天 404」，落进表格却变成了
> 「这个源不可用」—— 两者差一个量级。
> 而一旦写成表里的一个 ❌，**没人会回头质疑它**。
...
| | |
|---|---|
| `rn > 50` 返回**空列表**且 `errno=0` | 交替三轮完全一致。照着「多要一点总没坏处」调到 100，得到的是一个**永远说今天没新闻**的 provider，不报错。⇒ `MAX_PAGE_SIZE` 封死 50，越界在发请求之前就拦 |
| `level` 是**真信号** | 180 条实测 C=162 / B=18。这是新浪完全没有的 —— 那边 `is_focus`/`top_value`/`tab` 三个字段恒 0 |
| 名字像宝的结构化字段**基本是空的** | 360 条填充率：`stock_list` **1%**、`plate_list` **0%**、`tags` **0%**、`bold` 恒 0。`subjects` 94% 但那是**栏目名**，不是个股/板块归属 |

第三条是 `is_focus` 全零那一幕的重演：**字段名不是契约，返回值才是。**
⇒ 适配器**只取 `level`**，不碰其余 —— 拿 1% 填充率的字段做归因，
得到的是一个 99% 说「不知道」的功能，而它**看起来像个功能**。

### 🔴 `level` 缺失是 `None`，不是 0，也不是默认档

降级到新浪时这个源不提供 `level`。全链路保持 `None`：

- `NewsFeed.important_count` 返回 `None` 而不是 0
- skill 产出 `news.level.unavailable` 进 `missing[]`
- 卡上看得见「本次无重要性标记」

「0 条重要新闻」和「这个源不提供重要性」在人做决策时是两件事，
而**降级发生的那一次恰恰最需要看清楚自己少了什么**（R-3）。

### 🔴 静默阈值**按源查**，财联社那份是 `None`

`STALE_SEC = 600` 是拿新浪**盘中**实测分布定的。财联社手上只有非交易日样本
（09-25 中秋 / 09-26 周六），同窗口速率差 4 倍 ——
抄 600 大概率得到一盏**正常也红**的灯，而几次之后所有人都开始忽略它。

⇒ `STALE_SEC_BY_PROVIDER = {"sina_news": 600, "cls_news": None}`，
`None` 时**不判**（R-3：算不出来就说算不出来）。下一个交易日 09-28 实测再定。

### 修复 · 静默判据的文案会说一句它没验证过的话

顺手把「本系统尚无交易日历」改掉时，我写成了「交易日已由日历确认」——
**日历覆盖不到那天时那是假话**（`market_is_open` 会回退到纯 weekday 判据，
节假日照样为真，它的 docstring 明写要把这点说出来）。

改成跟着日历的三态走，并补了**两个方向**的守卫：
日历没覆盖时必须提节假日 / 日历确认了就不许再甩免责声明。
探针验过两边都能红。

### 修复 · 广告剔除被算进了「解析失败率」

第一版把 `is_ad` 过滤放在解析里，于是 3 行里 2 条广告就会报
「字段可能改名了」—— 把一个**正常**页面判成故障。

> 一个计数器量两件事 = 两件事都测不准。

广告被剔除是**源自己声明的**，解析不出来是**形状变了**。现在分开算。

### 修复 · 那条财联社探针探的是一个我们永远不会发的请求

`source_survey.py` 里 `cls_roll` 的 URL **没有签名**，判据是「正文里有 `data`」。
实测那条 URL 返回的正是 `errno=10012 签名错误` —— 而错误正文里也有 `data`
⇒ **签名坏掉的那天它仍然绿**。又一个 L-13。

改成现算签名，判据打在 `errno == 0` **且** `roll_data` 非空上
（只判 `errno` 会漏掉 `rn>50` 那种「成功但空」）。
三种失败——坏签名 / 无签名 / `rn=99`——现在都能红。

### 变更 · `NewsItem` / `NewsFeed` 搬进 `providers/news_item.py`

两个 provider 共用一份定义。让 `cls_news` 从 `sina_news` import 会写下一句
假话：**财联社的行依赖新浪的适配器**。与 `eod_bar.py` / `security_listing.py`
同一个做法 —— **归一化后的行属于 dataset，不属于某个源**。

`sina_news` 继续再导出这两个名字（下游一直从那里 import，多改一处就多一次
漏改的机会）。`_QUOTE_RE` 一并搬过去：判据是**文本**不是来源，
一个源一套正则就是 L-3。

### 变更 · `cn.news.flash` schema 升到 v2

行上多了 `level`。不升版本的话，旧分区与新分区在同一个 dataset 下形状不同，
而读取方拿 `row["level"]` 会在旧分区上 `KeyError` —— 那是**只在回放旧快照时**
才暴露的崩溃。读取侧用 `.get`。

### 已知问题

- **财联社的盘中静默阈值仍是 `UNKNOWN`** —— 等 2026-09-28（下一个交易日）实测
- `cn.market.limit_pool` 现在是**唯一**的单点源
- 金十快讯探活通过但**口径没核**，未登记（候选 ≠ 已登记）

---

## [0.9.13] - 2026-09-26

### 🔴 撤回 · 那个「网关看不看得见密钥」的判据是错的

[0.9.11] 写进文档、[0.9.12] 又重复了一遍的判据是：

> 网关 pid 的 `/proc/<pid>/environ` 里该变量出现 **0** 次 = 它看不到。

**那是错的，而且方向刚好反。** `/proc/<pid>/environ` 是 **`exec()` 那一刻**的
环境快照；OpenClaw 的 dotenv 是在进程起来之后把值写进 `process.env` 的
（`src/infra/dotenv.ts`：`env[key] = value`）—— 运行时对 `process.env` 的修改
**永远不会**反映到 `/proc/<pid>/environ`。

⇒ 那个数**恒为 0**，配没配成功都一样。它不是判据，是一句必然成立的废话。

实测证伪：网关 `environ` 里两个变量都是 `0`，而**同一个网关派生出来的子进程里
两个都读得到**（47 / 40 字符）。

这正是 **L-13**（守卫查的地方和它声称的地方不是同一个）——
而它比没有守卫更糟：**它会一直绿，而你以为已经验过了。**
我照着它得出过「agent 看不到 key」的结论，那个结论是假的。

⚠️ **「改完 `.env` 必须重启网关」这半句仍然成立**（本次换 key 时再次实测：
运行中的网关拿的还是旧值，重启后才换过来）。错的只是验证它的方法。

真判据是端到端 —— 让网关派生的子进程自己报变量长度
（`biga agent --message` 跑一次真实 turn）。

### 🔴 外层环境里已有同名变量时，`.env` 不会覆盖它

dotenv 的写法是「没定义才写」（`if (env[key] !== void 0) continue`）。
⇒ 从登录 shell 启动的进程带着 shell 里那份，`.env` 里更新过的值**进不去**；
而网关是 systemd 用户单元、**不读 shell 配置**，拿到的是 `.env` 那份。

**同一把 key 换了新值时，这两条路会用不同的值，而且都不报错。**
实测踩过：换完 key 在命令行测，用的还是 shell 里的旧值。

### 变更 · Serper 换了新 key，额度问题解除

[0.9.12] 记的「账户无余额」已解决。实测：命令行 10 条真实结果 / 3.3s，
经网关的 agent turn 同样 `success: true, count: 10`。

`docs/guide/private-skills.md` 同步改掉了那段判据，并补上「外层变量会盖住
`.env`」这条 —— 它是**验证方法**的一部分，属于「跑不通就是错的」那类文档。
...
> 🔴 **这条判据在 [0.9.13] 被证伪** —— `/proc/<pid>/environ` 是 `exec()` 时刻的
> 快照，dotenv 在运行时写 `process.env` 不会反映进去 ⇒ 那个数**恒为 0**。
> 「必须重启网关」这半句仍然成立，错的是验证它的方法。
