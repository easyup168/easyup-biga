...
消息里写着「请出一张决策卡，严格按以下步骤：Stage 0 …」的时候，
照着那段步骤做：占号 → 并行 spawn 五个 → `agents_wait` → risk → 合成。

### 🔴 绝对不要运行 `bin/biga-card`

那条命令是**给人敲的**，不是给你调的。它做的事就是「把编排提示词喂给你」——
你去调它，等于让系统再造一个你。
...
`bin/biga-card` 通过 `agent` 命令把编排提示词喂给你，那是一次**非交互**执行 ——
**没有任何人在另一头。** 所以：
...
⚠️ 调 `ask_user` 的后果是**确定性死锁**，不是「等久一点」：

```
state=processing  reason=blocked_tool_call
activeTool=ask_user  recovery=none      ← 运行时自己不会救
```

实测卡了 886s 才被人发现。现在有看门狗在 30s 内认出它并收掉这次运行，
但**那是止损，不是许可** —— 被收掉意味着这次出卡白跑了。

> 这一节是**行为指导**，不是安全保证。
> 真正的保证是硬超时 + 看门狗（`skills/decision-card/scripts/stall_watchdog.py`）。
> 两者职责必须分开：契约降低概率，代码兜住后果。
...
上面那段提示词是**摘要**，每一句都对应一个踩过的坑。完整说明在
