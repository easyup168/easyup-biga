...
```
0  通过
1  不通过 —— **确实观察到了违规**。去看代码 / 配置
2  判不了 —— **没查成**。去看数据、环境、时机，不要去改代码
```

⚠️ 这两个非零码的区别是**排查方向**，不是严重程度。
把 `2` 读成「坏了」会让你一开始就走错方向 —— 最常见的 `2` 是
「报告跑早了，Supervisor 那一轮还没写进 trajectory」，等十几秒再跑就好。

```bash
python3 tools/verify/latency_report.py --parallel-check; echo "rc=$?"
```

口径定义在 `tools/verify/_verdict.py`（唯一实现）。六个工具都从那里取码，
由 `tests/test_verify_exit_codes.py` 用 AST 钉住 —— 不许再出现裸数字。

> 三态在**每一层**都要是三态。渲染层写着「🔶 判不了」、退出码却是 `1`，
> 等于没做：最常见的读法是「跑完没报错就是过了」，读的正是退出码。

⚠️ **`bin/biga-card` 不在这套口径里 —— 它是生产者，不是检查器。**
它自己那套码见下一节「出卡命令的退出码」。

🔴 这里原来又抄了一份它的退出码表。两份同一规格就是 §9 **L-3**：
改了一份忘了另一份时，剩下那份仍然读起来像权威。
...
```bash
cd ~/.openclaw-biga/workspace
echo "为什么停：<写一句，解除的人要看>" > .biga-card-stop   # 装总闸
rm .biga-card-stop                                          # 解除
```

存在这个文件 ⇒ **出新卡一律 `exit 3`**，在任何花钱的动作之前。
**只拦花钱的** —— `--list` / `--show` / `--check` 照常，事故当中最需要的就是看现状。

为什么是文件而不是环境变量：环境变量只对「记得导出它的那个 shell」有效，
而失控时的触发源是几十个各自独立的进程组，它们的 env 你控制不了。
文件是所有路径都绕不过去的同一个事实。

### 还有一道自动的：单实例锁

```
🔴 已经有一次出卡在跑，这次不再起（单实例锁）。
```

`bin/biga-card` 用 `flock` 保证同时只有一次。2026-09-21 的递归事故里
（`architecture.md` §9 **L-14**）这道锁能把 195 次调用压成 1 次。

⚠️ 用文件锁而不是 `BIGA_CARD_DEPTH=1` 这种标记：**env 传不过网关** ——
...
`4` 是钱已经花了、卡已经落库，只是不能当证据用。

---

## 四点五、🔴 出卡是要花钱的，有闸门

一次真实出卡 = **约 3 分钟 / $1.2~1.4**。接飞书之后手机上一句话就能触发它，
而在那之前每次都要人手工敲命令 —— **误触的代价从零变成了 $1.37**。

闸门放在 **Stage 0 占号**这个咽喉点上（任何触发路径都绕不过它）：

| 规则 | 默认 | 为什么是这个数 |
|---|---|---|
| 最小间隔 | 400s | 实测单次 172~198s，留一倍余量。比一次运行还短的间隔没有意义 |
| 当日上限 | 20 | 不是为了省钱，是**让失控可见** |
| 「上一次还在跑」 | 600s 窗口 | 🔴 必须有窗口 —— 库里有冒烟测试留下的占号，它们永远不会出卡，不设窗口闸门会**永久关死** |

被拦的样子：

```
🔴 出卡预算闸门拒绝了这次请求：
  · 距上次占号只有 55s，最小间隔 400s（BIGA-20260921-022，by=test）。…
  · 还有 1 次运行没出卡且在 600s 窗口内（BIGA-20260921-022）。…

  确实要跑就加 --force（并说明理由），或调 BIGA_MIN_GAP_SEC / BIGA_DAILY_CAP。
```

⚠️ **被拦的时候不会占号** —— 否则连点十下就把当日配额用完了，而一张卡都没出。

⚠️ 它拦的是**新占号**，不是已经在跑的运行。已经花掉的钱拦不住。

### 顺带看一眼「孤儿 spawn」

`budget_report.py` 还会报**跑了但进不了任何卡**的 spawn：

```
🔴 孤儿 spawn 4 个 —— 跑了但进不了任何卡（约 $0.40~$0.60）
     19:31:52  market      无决策号
     19:31:52  sector      只带临时号 -000（落库会被拒）
     根因多半是 Stage 0 顺序反了：**先占号，再 spawn**
```

🔴 **`spawn_check` 查不到它们** —— 那个按决策号查，而孤儿的特征
恰恰是**没有号可查**。两个检查方向相反，缺一不可：

| 检查 | 问的是 |
|---|---|
| `spawn_check <号>` | 这张卡上的 agent，真的被 spawn 了吗 |
| `budget_report.py` | 被 spawn 的 agent，最后进卡了吗 |
