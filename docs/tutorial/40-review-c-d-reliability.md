...
## ⏩ 后续变动（2026-09-24，二轮对抗性复核）

上面「验证」一节写的 `# 1375 条，全绿` 在写下的那一刻就是错的，不是后来
过期——本章新增的 `docs/tutorial/40-review-c-d-reliability.md` 这个文件
本身触发了 4 条文档扫描类测试的新参数化 case，同步徽章时漏算了"写教程
本身也会让测试条数变化"这件事。另外，第 40 章记录的五处修复里，C-1 与
D-2 各自被发现了更深一层的问题（C-1 的安全论证范围小于代码判断范围、
D-2 的 reaper 全仓零调度方），详见第 41 章。
