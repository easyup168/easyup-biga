# 第 51 章 · 让这个仓库变成一个装得上的包（F 节 · 批 U-II）
...
> ruff/mypy 基线（批 U-IV）；包自足本身见[第 50 章](50-package-self-containment.md)
...
那条是同一件事：数字必须描述一个真实状态（第 50 章开头那个测试条数徽章，
...
文件），外加它带进来的第 50 章与本仓库已有的第 50 章**同号**。

处理：**不去解别人的 merge**（第 48 章那次事故就是这个形状），把本批挪进独立
worktree 提交。这是仓库自己的先例（第 35 章「并行撞树第二次 ⇒ worktree 隔离」）。

⚠️ 章节号因此留了一个已知的待解冲突：本章在 `u-two` 分支上是 51，
而 incoming 的那一批也需要一个号。按仓库先例（第 36 章）**按落地先后处理，
后落地的那个改号**。
