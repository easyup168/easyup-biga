...
| 50 | [让包真的自足（F 节 · 批 U-I）](50-package-self-containment.md) | 外部评审 F 节批 U-I：批 H-I/H-II 搬的是「对外接口」，包**内部**仍写着 `from _contract import ...`，靠 `pythonpath` 同时挂 `skills/` 与 `src/` 才解析得开；🔴 要害不是不优雅，是**全量回归测不出来**——实测把一行改回旧写法，`test_store.py` 47 条全绿，同一刻只挂 `src/` 的隔离 import 当场 `ModuleNotFoundError`（测试环境路比真实安装环境宽 ⇒ 全绿只证明「在宽路上能跑」）；真正的交付物是**隔离判据**不是那 9 行 import；顺带删掉 `tradetime.py` 一处 `sys.path.insert`——它不属于 U-III，因为 U-III 要判断「这行还有没有用」而它的唯一消费方就是同一次编辑删掉的那行，留着只会留下一条指向已不存在机制的注释；守卫判据必须 **AST 不能 grep**（文档字符串示例与相对导入的下划线兄弟模块会误报，函数内惰性 import 与 `__import__("…")` 会漏报，四种形状 grep 错三漏二）；🔴 **惰性 import 要单独一条真把函数调起来的测试**——sabotage 只坏惰性那行，模块级 import 那条全绿；反向自检最容易漏「隔离环境里 `skills` 确实不可达」，少了它隔离测试可能在验证恒真命题；基线开工就是红的（上一提交加文档未同步条数，按 `docs/**/*.md` 参数化每份 +3），先证明「这红与本批无关」再往下做 | ✅ |
| 51 | [让这个仓库变成一个装得上的包（F 节 · 批 U-II）](51-packaging-metadata.md) | 外部评审 F 节批 U-II：`pyproject.toml` 补 `[build-system]`+`[project]`，五个问题全部**核实**不凭空定；`skills/` 不进包三条理由任一都够（连字符目录做不成包 / 无仓库外消费方 / 装进去等于占 site-packages 这个共享命名空间的通用名＝R-2 的形状）；依赖是**扫出来的**——结论零依赖，网络走 stdlib `urllib.request`；🔴 **探针前提不成立时重新设计而不是跳过**（没有依赖项可删 ⇒ 换成「声明⇔代码双向一致」）；版本写 `0.3.0.dev0` 不写 `0.2.0`（写已发布版等于宣称「这棵树就是那个 tag」），代价是版本号有两个出处 ⇒ 守卫判「不矛盾」非「字面相同」；`requires-python` 宁可窄，未经测试的下界是空头宣称；console scripts **裁定不做**且守卫钉的是**当初的前提**（`tools/` 不是包、8/10 脚本仓库绑定）而非结果；🔴 **探针当场证伪我自己刚写的判据**——按 glob 重算那版只看 `include` 漏了 `exclude`，改成**问 setuptools** 而不是补写 glob；陈旧 `build/` 会让配置改动看着没生效（非 pip 缓存）；🔴 `pip install` 造出的 `build/lib/` 第二份拷贝让降级扫描把它当成**契约的第二份实现**，五条守卫集体误报——修在**扫描器**里而非测试的复制清单里，且沙盒不再复现 ⇒ 必须另有单独测试钉住；editable 安装**测不出打包范围**，`pip install .` 才是验收；并行撞树第三次（撞上进行中的 merge）⇒ 不解别人的 merge，挪进独立 worktree | ✅ |
