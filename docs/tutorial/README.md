...
| 36 | [飞书出卡变成结构性 trigger](36-inbound-trigger.md) | 确定性编排批 G-II（Inbound Trigger）：根子修法是「main 收不到这类请求」不是「教它转发」（反事实：清空 main 的 prompt 这条路还能走）；飞书是 websocket ⇒ 没有能指向的 HTTP 端点 ⇒ 拦截只能表达成配置；`command-dispatch: tool` 让 `/card` 绕过 model，`command-tool` 不能指 `exec`（会把聊天框变 shell）；幂等键绑 `decision_ids`（每决策一行）不绑 `decision_runs`（会误伤重试）；异步用 `systemd-run` 把出卡拉出 agent 进程树 ⇒ entry_guard 判 HUMAN（走它本就留的「外部 Trigger」门，不开口子）；人工 CLI 与飞书走同一个 `bin/biga-card`；`tools.deny:[ask_user]` 只给非交互流水线 agent、main 不在名单、名单从 `_contract` 派生；并行批撞共享工作树的复发提醒（第 28 章纪律） | ✅ |
