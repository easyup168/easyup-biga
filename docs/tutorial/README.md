...
| 57 | [空列表不等于安全（裁定 16 · 批 5，收官）](57-unknown-is-not-safe.md) | 🔴 **R-3 最坏的形状不是报 UNKNOWN，是把未知显示成安全**：上游一个阈值字段都没有时，`tripped_thresholds=[]` 在卡面上读起来是「比过了没触发」，实际是「没东西可比」⇒ 改成不产出该字段 + 进 missing；改完 20 条编排器测试变红，🔴 **先分清行为错了还是夹具不现实** —— 实测生产 23 次决策 22 次有 3–4 个可比字段，红的是夹具；顺着「测试红了就是改错了」退回去，留下的就是那个洞；契约扩到 `observed`（「我观察到的」指不回任何响应时和「我编的」一样），🔴 **规则能不能收紧取决于被约束对象有没有合法出路** —— 批 4 的 `derived_from` 是前提，顺序不能反；测试帮助函数的默认值应当构造**合法**对象 | ✅ |
| 58 | [超时之后没人喊停（G 节 Live Acceptance 第 12 项）](58-cancel-on-timeout.md) | 核实"无残留 Session"这一项时读代码发现真缺陷，不是漏测：`ad.wait(handles, self._stage_timeout(...))` 里后者抛异常时，Python 先算参数再调用，`ad.wait()` 根本没被执行，三处（Stage 1/risk/synth）刚启动的 handle 既没等过也没取消；🔴 更常见的第二条路——`ad.wait()` 正常返回但单个 handle 到期，运行时的 `TIMEOUT` 状态是**本地生成**的，不代表对方真的停了；修法接进 Stage 1 已有的「start 失败就 cancel 兄弟」防线（批 C-III），不是另起一套；新增的两条测试复用既有 `FakeAdapter.cancelled` 记账，无需新测试基础设施 | ✅ |
| 59 | [ACK 抢在闸门前面许诺了一件闸门几秒后会否决的事](59-ack-before-gate.md) | 真机触发飞书 `/card` 撞见（不是读代码找的）：CLI 触发一次真实出卡 10 分钟内又用飞书触发一次，被预算闸门拦下（390s vs 400s 最小间隔），但飞书那侧收到"正在出卡……不用等"——三分钟后什么都没有；根因是 `launcher()` 只保证 systemd-run 瞬态单元排上去了，不等它跑完，闸门判断在单元**内部**才执行，ACK 却在排上去那一刻就发了；🔴 **"异步任务已提交"和"异步任务会成功"是两个不同时间点才能确定的事实**，只知道前者就说后者是提前下注；修法是拉起前自己先问一遍只读的 `check_budget()`，会拒就不拉起、直接说真话；`exclude_decision_id` 排掉刚占的号——不排除会「跟自己比 gap 恒为 0」，同一个坑 `bin/biga-card` 自己那道检查早就踩过；一条既有测试背靠背触发两个 trigger 恰好撞上新检查，短路掉闸门让测试回到本来焦点（去重身份，不是预算时序） | ✅ |
| 60 | [H 节 Baseline 冻结——给这次升级盖一个章](60-baseline-freeze.md) | H 节 8 项（tag / Schema 说明 / 回滚说明 / Version / 两个 Hash / 更新 Architecture / Roadmap）不依赖 G 节独立 sign-off 是否已发生——证据集齐与形式复核是两个判据；Hash 而不是原文抄进文档（`$HOME` 真实路径不能进公开仓库），且先把路径替换成 `~` 再算，换机器/换用户名 hash 不漂；Schema 版本说明只放一句话摘要指回 `schema.py` 注释，不重新叙事；老实承认"没有自动化迁移前快照"这个缺口，17 条迁移全是 additive 只是侥幸不是设计能力；🔴 三个坑：`.parents[1] / "tools" / "verify"` 这种链式写法把既有 U-III 守卫误判成"挂的是仓库根"（守卫只认前半段的下标模式，不管后面还接了别的路径拼接），改成先赋值给命名变量再引用；往文档末尾插新章节时 `old_string` 没覆盖到表格真正最后一行，把表格从中间切开；🔴 **最重要的一个**——查到三份文档里已经有一段自称"新会话独立复核"的详细记录（DB 核对、三处 sabotage、数字更正），逐条重新核实全部为真，但 `git log` 证明它不在任何一次提交里，且这次会话唯一的提交干干净净没这些字——**内容多详细都证明不了它真的来自另一双眼睛**，两种可能（用户真开了新会话 / 同一条线索换了口吻自称新会话）从文字本身分不出来；先把三份文档改成"待确认"，再直接问用户，确认属实后照答案改回"已关闭"；余震——问完之后同一行**又被覆盖了一次**，查会话列表才想起当时有 31 个并行会话在跑，是那个新会话前后脚保存同一个文件，纯属"最后写的赢"，不是冲突 | ✅ |
| 61 | [给数据集上户口（Phase 3 · P3-0）](61-dataset-registry.md) | 外部参考骨架给 `cn.trading_calendar` 写的 `primary_provider` 是按文档写的**官方源**，实测库里 1326 行全部来自另一个源、官方那条适配器有测试有导出却**零生产调用方**（部署环境连不通）⇒ 照抄等于注册一个在这台机器上一行都没产出过的主源，连它的切片键也只对那个端点成立；名册的范围是「今天真的在跑的全部 7 组」不是「这一期要做的那几个」——收一半等于立刻有了第二份答案，而注册表是后来的那份、它会输；没探活过的数据集**不进**注册表（占位符会让人以为这条链已经有人管了）；`provider_id` 取 `raw_market_snapshot.source` 的前缀而不是模块名，否则同一个源有三个名字；孪生清单要**派生**不要对账（`supported_datasets` 从 dataset 侧算出来，顺带答出「它挂了会影响什么」这个今天没别处能答的问题）；🔴 **探针抓到两条结构上不可能红的守卫** —— 唯一性断言打在 `dict.values()` 上，而 dict 推导**已经把重复 key 静默吃掉了**，我在同一个 docstring 里写对了道理（「长度对不上是唯一痕迹」）却把断言打在看不见那个痕迹的地方 ⇒ 手写元组留成权威、dict 派生；🔴 **还原源码不够，`__pycache__` 也是工作区状态** —— 等长替换（`: 2,`→`: 1,`）+ 同秒还原骗过了 mtime+size 的 pyc 失效判据，源码 sha256 逐字节一致、探针如实报「已还原」，随后测试却红在一个源码里根本不存在的值上；守卫判据方向选反会既漏又噪（`"em:` 不在 provider 模块里、而那里有 `"https:` 之类噪音）⇒ 改成注册表→源码单向；名册必须有**行为上**的消费方（退出码 + 可解析输出）⇒ `bin/biga-data` 从 P3-1 拉到 P3-0，且它不能叫 `biga data`（`bin/biga` 是 openclaw 的纯 passthrough）；顺手写下的一句 `hasattr` 空断言 —— 字段最终没装，它退化成重复上一条，**不是错的，是什么都没查** | ✅ |
---
