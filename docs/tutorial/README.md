...
| 43 | [把一个坑填了两次的地方，填第三次：动态同名模块 monkeypatch 错位](43-module-identity-idempotent-load.md) | 外部评审 A 节：批 J-II（教程 32 章）只修了 `test_run_id_capture.py` 一处的"无条件覆写 sys.modules"坑，全仓还有 11 处一模一样的复制体从未被推广修复；真机复现 `pytest tests/test_orchestrator.py tests/test_facts_split_e3.py` 会红、反序不会；🔴 只有 `card_ops`/`risk_check` 是 `orchestrator.py` 直接 import、真有生产碰撞证据的，其余五个按评审点名的**模式**统一修，不因为"目前没坑到人"而留着；修法是把手写的加载逻辑补回标准 `import` 本该有的幂等语义（已加载就复用，不重新覆写）；新回归测试绕开"顺序"本身——直接断言两个引用是不是同一个对象；🔴 **sabotage 也要用对触发顺序**——顺序选反了，全绿证明不了任何事 | ✅ |
