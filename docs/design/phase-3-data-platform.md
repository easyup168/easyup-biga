...
### 3.7 `tools/verify/source_bundle.py` —— 出口条件 23 的那几步「操作」

出口条件 23（source ZIP 恢复 + replay）的**判据**早就写好了
（`data/drills.py:source_zip_replay_drill`），但它前面那几步 ——
把一次决策引用到的字节打成包、摊回一棵干净的树 —— 一直只存在于口头描述里。
于是这项演练**从来没被跑过**。不是因为难，是因为每次都要重新想一遍
「URI 到底是相对谁的」，而这个细节答错一次，回放会报「文件不存在」，
读起来像是数据坏了。

⇒ `export` / `restore` 两个子命令把那几步固定下来。三条硬规矩：

| 规矩 | 为什么 |
|---|---|
| **只带这次决策引用到的字节**，不扫整棵 `data/` | 多塞进去的文件会让回放在一棵本不该跑通的树上跑通 —— 那把「我恢复对了」和「我什么都带上了」混为一谈 |
| 库用 `VACUUM INTO` 取一致副本，**不是 `cp`** | WAL 模式下 `cp` 会漏掉尚未 checkpoint 的事务，而漏掉的部分**不报错**：库能打开、能查，只是少了最近几条 |
| `storage_uri` 是绝对路径 ⇒ **拒绝导出** | 绝对路径搬不动。恢复出来的库仍指着导出那台机器上的目录，而那个目录多半还在 ⇒ 回放读到**原树**的字节并报 PASS。「文件不存在」反而是好结局：至少它红了 |

🔴 **恢复出来的树上，不能用 `bin/biga-data` 跑回放。**
那个壳启动时 `cd` 回仓库根（为了让默认的 `data/` 相对路径成立），
而分区 URI 是按**进程工作目录**解析的（`FileStore.verify_file_hash` 用
`Path(uri)`，与 `--data-root` 无关 —— `FileStore.root` 只用于写）。
实测（2026-09-26）：把恢复树里一个 Parquet 删掉，经 `bin/biga-data`
跑出来仍然 `passed=True`。

⇒ 演练结果里因此记下 `cwd` 与 `db_path`：**这两个值才决定读的是哪棵树**。
不自动判定两者算不算「同一棵树」（目录布局不止一种，猜错会把合法调用判红），
但账本必须回答得出「当初读的是哪棵树」。
