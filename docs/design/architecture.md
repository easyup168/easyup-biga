...
| Stage | 内容 | 并行度 | 预估 | Phase 1 实测 | Phase 2 实测 |
|---|---|---|---|---|---|
| 0 | Supervisor 拆任务、发出 spawn | — | 5-10s | **8.0s** ✅ | **9.8s** ✅ |
| 1 | market / sector / news / technical / emotion | **×5 并行** | 20-40s（取最慢那个） | **36.0s** ✅（只有 emotion） | **31.2s** ✅（4 个并行） |
| — | 子 agent 交回到 Supervisor 的空档 | — | 未估 | 1.0s | 0.0s |
| 2 | risk / discipline | **×2 并行** | 15-25s | 未上线 | **16.4s** ✅（只有 risk） |
| 3 | Supervisor 合成 + 渲染 Card | — | 20-30s | **30.0s** ✅（压线） | **13.6s** ✅ |
| | **合计** | | **60-105s** | **74.8s** | **71.0s** |
