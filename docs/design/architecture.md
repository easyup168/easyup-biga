...
| **L-7** | **死配置**：因子/规则因命名空间不一致而交集恒空，长期零命中；或阈值判断在其作用域内恒真 | 它照常参与计算，只是永远不生效 | **可达性巡检**：每日报告「配了但从未命中」的规则与因子。⬜ **未建** —— 而 `risk_check.py` 的 6 条阈值里已有 3 条从未被任何测试触发过（F7）|
...
⚠️ **L-10 与 L-11 是这张表里仅有的两条在本项目自己踩出来的**，其余九条继承自那套长期运行的系统。
