...
│  data  : 它的行情库 655MB     │   │  data  : biga.db（自建）      │
...
上一次同类 PATH 问题让 `intraday_sector_monitor` 推送失败 **600+ 次无人察觉**。
...
- ⚠️ **v22.22.0 这个 node 本身必须保留** —— `skills/douyin-card/scripts/render_daily_review.py:128`
  硬编码引用 `v22.22.0/lib/node_modules/puppeteer`
...
2. 需要**跨机**访问同一份事实层
3. 单表 > **5000 万行**（现系统最大表 `stock_daily` 跑了 6.5 个月才 233 万行）
...
理由：判断逻辑散进 skill = 第二套口径，现系统为此栽过（`market_routing` 28 处实现 17 处错）。
...
现系统 6.5 个月最贵的产出不是代码，是这些。每条都配一个在 Phase 1 就建立的机制。

| # | 教训（现系统实证） | 新系统的机制 |
|---|---|---|
| **L-1** | 「写了但零消费方」累计 **7 次**，含整模块 `BridgeWatchdog`、以及被 8 处引用为权威却从无调度器的 `backfill_order_channel` | **产消对账测试**：`tests/test_no_orphan_producer.py` —— 任何写 DB 的模块必须在 registry 或调用图中存在读取方，否则红 |
| **L-2** | 闸 fail-open 静默放行：`monitor_pool` 表空放行、R019 未知 key 被忽略导致规则退化成无条件命中、焊死 75 个交易日 | **买入侧一律 fail-closed**；`UNKNOWN` 独立于 `PASS`；算不出来必须进 `missing[]` 且上 Card |
| **L-3** | 同一判据有多份实现：`market_routing` 全仓 28 处、17 处错，且错法全是静默 | **判据单一实现 + AST 钉死**。代码→交易所、涨跌停、费率等一律走 `_contract` / `_store` |
| **L-4** | 超额基准 v1（逐日截面中位连乘）被安慰剂检验证伪，全部正结论撤回 | **任何基准上线前必须过安慰剂**：随机股喂生产路径，`|t| < 1.96`。`tools/verify/placebo.py` |
| **L-5** | 统计结论「估计量依赖」：同一数据簇等权 t=−1.00 不显著 / CRVE t=−3.23 显著 | **报结论必须写明用哪个 t**，且 n<30 一律标 underpowered。禁止「只报让结论存活的那个」 |
| **L-6** | 文档漂移静默：CLAUDE.md 写 它的行情库「130MB/30 表/188万行」，实测 **655MB/110 表/381万行** | **关键事实由测试生成或校验**，不手抄。`tests/test_docs_facts.py` |
| **L-7** | 三个死因子零命中 4 个月（命名空间分裂：主线取行业名、候选取概念名，交集恒空） | **可达性巡检**：`tools/verify/reachability.py` 每日报告「配了但从未命中」的规则/因子 |
| **L-8** | 账本幻影行 33%：记了未发生的成交 | **raw 层永不改写**；状态变更追加而非 UPDATE |
| **L-9** | 真钱下单四铁律（超时预算不倒挂 / 盘后必 FIX / 探针 fail-closed / 止损不贴笼子下限），每条都是一次事故换来的 | **Phase 1 不下单，但这四条原样抄进 `docs/design/live-order-rules.md` 冻结**，将来开下单时直接实现，不重新踩 |

⚠️ L-9 是 §0.1 裁定 2「将来都有自动下单」的直接落点 ——
**不实现，但先把知识存进去**，代价近乎为零。
