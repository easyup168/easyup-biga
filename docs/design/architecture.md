...
| `src/easyup_biga/data/` | Phase 3 · P3-0 新增。`DATASETS`（7 条）与 `PROVIDERS`（4 条）是**数据集/数据源名册的唯一源**，`bin/biga-data list|providers` 读它。🔴 名册的范围是「今天真的在跑的全部」而不是「Phase 3 要做的那几个」——收一半等于「系统里有哪些数据集」立刻有两份答案，而注册表是后来的那份。`provider_id` 取 `raw_market_snapshot.source` 的**前缀**（东方财富是 `em` 不是 `eastmoney`），否则同一个源在源码、库、注册表里有三个名字。「谁支持哪些 dataset」**派生**不手写（`datasets_of()`）——它与 dataset 侧的 primary/fallback/validation 是一对孪生清单，两处各写一份必然漂。🔴 手写的是**元组**、dict 另外派生：dict 推导会静默吃掉重复 id，唯一性守卫打在 `.values()` 上结构性不可能红（L-13，探针当场抓到）。契约里 `TIMEOUT` 退 **2** 不是 1 —— 对齐 `tools/verify/_verdict.py` 的「2 去查数据、1 去查代码」（Phase 3 裁定 12）|
