...
> ⏩ **2026-09-26 更正：这个标题本身低估了差异。**
> 不只是「少两个字段」—— 备用源换的是**整套分类体系**
> （行业 496 申万式细分 → 84 证监会门类）。见 [§3.13](#313--板块备用源换的是整套分类体系不只是少两个字段)。
> 下面这段仍然成立，但它描述的只是差异的一部分。
...
### 3.13 🔴 板块：备用源换的是**整套分类体系**，不只是少两个字段

**裁定（2026-09-26）：板块以东财为主源。**

A 股板块数据是个课题 —— **行业、概念各家的分类都不一样**。实测：

| kind | 东财 | 新浪 |
|---|---|---|
| `industry` | 496 个申万式细分<br>`BK1456 其他家电Ⅲ` / `棉纺` / `焦炭Ⅱ` | **84 个证监会门类**<br>`hangye_ZA02 林业` / `畜牧业` / `农业` |
| `concept` | 500 个概念 | 175 个概念<br>`gn_hwqc 华为汽车` / `gn_BCdc BC电池` |

⇒ 备用源**不是「同一个榜的另一个来源」，是另一个榜**。
降级那天卡上会说「领涨板块 = 林业」—— 那是证监会门类里的一个大类，
对短线基本无用，而它和主源那个「棉纺 +8%」根本不是一回事。

**为什么仍然以东财为主**：各家分类不一致时，**跨天可比性比可用性更重要**。
一个换了口径的「备用源」对这个事实不构成真正的备用 ——
昨天的第一名和今天的第一名不在同一个榜上，趋势就无从谈起。

**为什么备用源仍然保留**：有总比没有强，但**必须声明**。
`sector-calc` 现在逐 kind 产出 `<kind>_taxonomy` 事实，
并在与主源不同时进 `missing[]`（不是 `warnings[]` ——
警告会被读成「注意一下」，而这里的事实是**这个榜不能拿来比**）。

⚠️ 同机已有实例的做法可以对照：它的板块底层行情**也全部来自东财**，
主源失效时的降级提示指向一个**商业数据 API**（另一条完全独立的风控面），
而不是换一家免费源 —— 那正是「不换口径」的另一种实现。
...
| 5 | **个股所属板块**没有本地库 | 成分会变动（调入调出、新概念成立）。同机已有实例的做法是**每周六一次定时任务**从东财下载一份成分数据落本地 DB。我们迟早需要同一份东西 —— 没有它，「这只票属于哪个板块」只能在每次取数时现问，既慢又无法回放**历史上那天**的归属 |
| 6 | 东财是**四个 dataset 的共同风控面** | `breadth` / `board_snapshot` / `limit_pool` / `security_master` 都走东财 ⇒ 一次封禁四个一起降级（2026-09-26 实测）。限流已补齐（v0.9.15），但**同源集中**本身是个结构性风险 |
