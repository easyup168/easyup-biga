#!/usr/bin/env python3
"""install_reap_timer.py —— 装 `stale-run-reaper-biga.timer`，定期跑 `bin/biga-reap`。

对齐外部评审 §14 / D-2：`stale_run_reaper.py`（批 D-2）早就能扫非终态太久的
run 并安全收成 TIMEOUT，缺的只是「谁、多久调一次它」——2026-09-24 对抗性
复核指出这个缺口是当前就在咬人的（不是面向未来）：全仓 grep 不到任何调度方
调用 `stale_run_reaper`，CLAUDE.md 自己那条「新增写数据模块必须有被证明的
读取方，判据是调度命令的字面量」在这里落了空。

本文件与 `install_notify_timer.py` 几乎同构（同一套 R-2 判据、同一套
install/uninstall 骨架）——这是有意的对称，不是复制粘贴的疏漏：两个都是
「BigA 自己的 systemd 定时器，跟 OpenClaw 的 profile/gateway 无关」，
适用同一条 R-1 例外理由（不经过 `bin/biga`）与同一条 R-2 判据（单元名带
`-biga` 后缀）。两段小重复 好过 为两个只共享骨架、单元名/路径各自独立
硬编码的安装脚本硬拗一个共享抽象。

用法
----
    python3 deploy/openclaw/install_reap_timer.py            # dry-run：只打印会做什么
    python3 deploy/openclaw/install_reap_timer.py --apply    # 真装 + enable --now
    python3 deploy/openclaw/install_reap_timer.py --apply --uninstall  # 卸载
"""
...
UNIT_SERVICE = "stale-run-reaper-biga.service"
UNIT_TIMER = "stale-run-reaper-biga.timer"
...
def install(*, dry_run: bool = True, runner: Runner = _real_run,
    ...
    改 `deploy/openclaw/stale-run-reaper-biga.timer` 源文件，重新跑一次本脚本。
