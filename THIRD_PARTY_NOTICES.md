...
non-stdlib, non-BigA-internal name against the environment's installed
...
this repository (checked `skills/`, `bin/`, `tools/`, `deploy/` for common
markers — "adapted from", "vendored", "based on ... license", inline
third-party copyright headers — none found).

Two third-party Python packages are actually imported by BigA's own code,
...
| License | `MIT` | `MIT` |
| Usage | Runtime dependency — `deploy/openclaw/apply_config.py` reads/writes OpenClaw's YAML config | Development/test-only — imported exclusively under `tests/`; not distributed with BigA, never imported by the actual pipeline (`bin/biga-card`, `bin/biga-notify`, etc.) |
| Modified | No — installed from PyPI as-is | No — installed from PyPI as-is |
| Required notice | Standard MIT copyright notice, carried in the installed package's own metadata; not reproduced here since BigA does not bundle or redistribute the package source | Same as PyYAML |
...
Everything else BigA's own code imports is either the Python standard
library (`urllib.request`, `http.client`, `sqlite3`, `json`, `dataclasses`,
`ast`, …) or BigA's own internal packages (`_contract`, `_store`, `_sources`,
`_runtime`, `_snapshot`). This project deliberately avoids third-party
HTTP/data-wrapper libraries for its provider adapters — see
`skills/_sources/eastmoney.py`'s docstring for the concrete incident that
...
inside this repository) — not bundled, vendored, or redistributed by BigA's
...
BigA's software license does **not** grant rights to market data, financial
data, news, research reports, filings, web content, or other third-party data
retrieved through BigA.
...
verifiable from BigA's own adapter source code today; it is not a legal
review of any provider's terms of service.

| Provider | Interface | Auth in adapter code | ToS / rate-limit reviewed |
|---|---|---|---|
| 深交所 SZSE `monthList`（`skills/_sources/szse.py`） | **Official** exchange endpoint | None | Not reviewed |
| 新浪财经日线（`skills/_sources/sina.py`） | Unofficial, reverse-engineered public endpoint | None | Not reviewed |
| 新浪财经 7x24 快讯（`skills/_sources/sina_news.py`） | Unofficial, reverse-engineered public endpoint | None | Not reviewed |
| 腾讯行情（`skills/_sources/tencent.py`） | Unofficial, reverse-engineered public endpoint | None | Not reviewed |
| 东方财富股池/涨跌家数（`skills/_sources/eastmoney.py`） | Unofficial, reverse-engineered public endpoint | None | Not reviewed |

"Auth in adapter code: None" means BigA's own code sends no API key or
...
third-party components of BigA**. Do not copy their code until the applicable
