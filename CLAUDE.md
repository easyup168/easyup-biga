...
| 契约 / 数据层 | `_contract` 四条铁律构造时拒绝；`_store` schema **v12**，十张表，只追加由触发器强制（v4 时这句话是**假的** —— 分配器没有触发器，五取四；v6 起 `agent_verdicts` 加 `kind` 列区分事实/判断/合体；v9 收敛 `run_id` 三同名；v10 `run_id` capture 贯穿全链；v11 加 `ux_fact_per_task_agent`——一个 `(task_id, agent)` 至多一份 fact 原件（批 F）；v12 加外发通知 `notification_outbox` / `notification_deliveries` 两张只追加表（批 G-I）） |
| 测试 | 1241 条 |
