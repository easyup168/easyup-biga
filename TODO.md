...
来源：`docs/external/2026-09-21-adversarial-review-phase2.md`（23 条，钉在 commit `f7f7d1e`）。
🔴 **这份评审是外部只读材料，修复进度记在这里，不要回头改那份报告。**

⚠️ 它与另一份独立评审（`easyup-biga-phase2-code-review.zip` → FIX-01…06）
**互不知情却在多处指向同一根因**。收敛本身是比任一份报告更强的信号 ——
重叠的那几条优先级要往上提，不要当成「已经修过了」。

| 状态 | 条目 |
|---|---|
| ✅ 已修 | **F1** 分配器缺只追加触发器（schema v5）· **F20/F21** worktree 污染 AST 扫描（`tests/_scan.py`）· **F22** 文档测试条数漂移 |
| ⬜ 高危未修 | **F2** 隔离检查在目标不存在时判真 · **F3** 五个新 specialist 没有 spawn 自动核验 · **F4** raw 层 `as_of` 分裂只打了补丁 · **F5** 技术面缺量级围栏 · **F6** 资金全零时「第一名」是任意行 · **F7** §9 里两条护栏从未建过 |
| ⬜ 中危未修 | F8 F9 F10 F11 F12 F13 F14 F15 F16 F17 F18 F19 |
| ⬜ 低危未修 | F23 |

**下一个动手的是 F2。** 理由是评审自己给的：F1 投入产出比最确定（已做），
而**此刻风险最大的是 F2** —— 第一不变式的检查在「什么都没扫到」时报绿，
这正是本项目反复栽的那个形状。

> 🔴 **反复出现的形状（评审自己归纳的）：手工维护的名单。**
> F7 / F8 / F9 / F10 / F16 / F19 / F21 是它的七个独立实例。
> 修单条没用 —— 每处都要改成「默认纳入，例外自己举手」。
> F1 与 F22 这两条就是按这个改的，可以照抄。
