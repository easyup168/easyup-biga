...
| 分组 | 条目 | 怎么修的 |
|---|---|---|
| 决策身份 | F1 | schema v5，分配器补只追加触发器 |
| 自检工具自己 | F2 F18 F19 | 三态 `Result`、两处「无事可查」、I-1 判据合并成一份 |
| 看起来正常的错值 | F5 F6 | 抽出 `_sources/sanity.py` 量级围栏；解析层不再把缺失吞成 0 |
| `as_of` 归属 | F4 F15 F16 | raw 层逐源归属、源自己声明 `server_as_of`、证据锚到真实时钟 |
| 清单不会自己长大 | F8 F9 F10 F17 | 未登记即错、契约清单改扫目录、第二处名册对账、并发测试真起进程 |
| 守卫的边界 | F11 F12 F13 F14 | 守住后备防线的前提、间接 import 默认禁止、可判定的兜底、把边界印进输出 |
| spawn 证明 | F3 | 判据取自 stage 名单，接进 `bin/biga-card` |
| 文档与体验 | F7 F20 F21 F22 F23 | 幽灵文件守卫、`tests/_scan.py`、条数不漂移、库缺失给人话 |

**评审报告本身不改**（`docs/external/` 只读），进度只记在这里。
