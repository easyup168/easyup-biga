...
- [ ] 🔴 **当天日线几点发布** —— 「收盘后跑就对齐」这条推断已被实测证伪
      （15:21 跑，日线仍停在上一交易日；15:22 查源同样）。
      正在每 5 分钟探一次，记录到本机 scratchpad。
      测出来之后写回 `docs/guide/usage.md` 与 `phase-2-specialists.md` §3.11。

      ⚠️ 在测出来之前，**不要再给出任何「几点之后跑」的建议** ——
      上一条就是这么错的：听起来太合理，于是没人去验。
...
| | 来源 | 条目 | 状态 |
|---|---|---|---|
| 评审 ① | `easyup-biga-phase2-code-review.zip` | P1-1…3 / P2-1…3 | ✅ 全部修完（FIX-01…06） |
| 评审 ② | `2026-09-21-adversarial-review-phase2.md` | F1…F23 | 7 条已修，16 条未修 |
...
| 状态 | 条目 |
|---|---|
| ✅ 已修 | **F1** 分配器缺只追加触发器（schema v5）· **F7** 设计文档点名不存在的文件（见下）· **F20/F21** worktree 污染 AST 扫描 · **F22** 测试条数漂移 |
| ⬜ 高危 | **F2** 隔离检查在目标不存在时判真 · **F3** 五个新 specialist 无 spawn 自动核验 · **F4** raw 层 `as_of` 分裂 · **F5** 技术面缺量级围栏 · **F6** 资金全零时「第一名」是任意行 |
| ⬜ 中危 | F8 F9 F10 F11 F12 F13 F14 F15 F16 F17 F18 F19 |
| ⬜ 低危 | F23（`latency_report.py` 缺库时是裸 traceback） |

### F7 的实际规模比报告说的大三倍

报告点名 2 个幽灵文件。上守卫之后扫出 **6 个**，其中 4 个是
「由 `tests/xxx.py` 钉住」这种**陈述句**，而那些测试文件都不存在：

| 位置 | 声称 | 实际 |
|---|---|---|
| §2.4 树 | `placebo.py` / `reachability.py` | 从未提交过 |
| §2.4 树 | `runner.py` | 未建 |
| §1 | 「由 `tests/test_isolation.py` 钉住」 | 该文件不存在；真实是 `tools/verify/isolation.py`，**脚本，不在 pytest 里** |
| §3.4 | 「由 `tests/test_skill_allowlist.py` 钉住」 | 🔴 **该文件不存在，也没有任何替代品** |
| §5 | `tools/cron/registry.yaml` | 未建 |
| §9 L-9 | `docs/design/live-order-rules.md` | 未建 |

- [ ] 🔴 **补 skills 白名单守卫** —— `agents.entries.*.skills` 是替换语义，
      漏抄一个技能**不报错，只是静默少一个能力**。
      `architecture.md` 声称有测试钉它，实际没有 ⇒ 这条风险当前完全无人管。

> **评审找到的是样本，不是全集。** 人去找的时候，也只找得到想到过的那几个 ——
> 这与「手工维护的名单」是同一个形状的两面。
> ⇒ 拿到评审结论后，**先给那一类上一条机器判据，再看它扫出多少**。

### 下一个动手的是 F2

理由是评审自己给的：F1 投入产出比最确定（已做），**此刻风险最大的是 F2**。

`Result.add(ok: bool, ...)` 只有两态，端口检查连「没人在监听」这一档都没有
（`clash=set()` → `not clash` → ✅）。⇒ I-1 这道第一关卡在真实网关不存在时
稳定报绿。**这违反 R-3（`UNKNOWN` ≠ `PASS`）本身**，而且它取代的旧脚本
`phase1_acceptance.py` 用的是三态 `PASS/FAIL/PENDING` —— 是一次退步。
F2 / F18 / F19 是同一个根，要一起修。
