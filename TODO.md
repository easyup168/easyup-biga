...
| F8 | 未登记 agent 的 stance | 仍然是「一张表 + 查表」，只是查不到时从放行改成报错 |
| F10 | 第二处名册对账 | **清单式检查只加固了这一个字段**，第三处名册出现时同样没人管 |
| F16 | `server_as_of` 统一接口 | 仍要靠人在新源上**记得实现它**（有守卫，但守卫也是一份清单） |

⇒ 这五条**不单独再开工**，但下次碰到同类结构时，先问一句
「还有哪些地方是同样的形状」，而不是只补眼前这一处。
