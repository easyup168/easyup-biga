...
- [x] 批 H-I · 三个基础设施包迁移 —— **独立复核通过（2026-09-24）**。
  ...
  - [ ] 🔴 **给后续批次的提醒**（批 3 完成时点出来，转告 U-II/U-III 的开工者）：
        裁定 16 批 2/3 往 `src/easyup_biga/domain/__init__.py` 加了新导出。
        U-III 逐一核实 `sys.path.insert` 时如果也要过一遍 `domain/__init__.py`
        的 `__all__`，别把它们当成"批 U-I 之前就有的"——它们比 U-I 晚落地。
        ⚠️ **2026-09-24 订正**（U-II 合并时用 AST 对两棵树的 `__all__` 实测）：
        是**五个不是四个**，且原记录里的 `input_ids_for` **全仓不存在**
        （`git grep` 零命中）。实际清单：
        `DERIVED_PREFIX` / `EVIDENCE_KINDS` / `SHA256_RE` /
        `resolve_provenance` / `underlying_source`。
        判据：`ast` 解出 `0e3cc07` 与合并后两侧的 `__all__` 集合再求差，
        不靠读 diff 认名字——这条记录的用途正是防止 U-III 误判，
        名字错了比没有这条记录更糟（它会让人去找一个不存在的符号）。
        `SHA256_RE` 确实算新增：它虽由裁定 16 批 1 引入，但进 `domain` 的
        `__all__` 是批 2（`a9a82bb`）才做的。
        另：`sys.path.insert` 的实测处数是 **128** 不是 82（U-II 开工时 grep）
  - [ ] 🔴 **解冲突丢换行：第 48 章那道守卫盖不住的变体**（2026-09-24 实测，
        `da824ad` 合并批 2 时真的发生了，`09d956c` 合并 U-II 时修掉）。
        症状：解冲突时把换行连同标记行一起删了 ⇒ 两个表格行 / 两个徽章 /
        两行树状图粘成一行。实测 5 处（README 3 / CLAUDE 1 / review-prompt 1）。
        **三道闸门全部合法地视而不见**，与第 48 章同构但原因各不相同：
        ① 第 48 章补的守卫查的是**冲突标记**本身，粘连行里没有标记；
        ② `pytest` 不解析 Markdown 结构；
        ③ 测试条数那条守卫照样通过——数字仍然在那条粘连行上找得到。
        ⇒ 合并后的树当时**真的跑绿过**，损坏就在里面。
        这次是靠「与合并前比行数」发现的（README -3 / CLAUDE -1 /
        review-prompt -1，正好对上 5 处）。
        可能的判据（还没想清楚该用哪个，别急着挑一个就写）：
        a. Markdown 表格行数在**非 CHANGELOG 类**文档里不该凭空减少；
        b. 一行里出现两个 `| ... |` 行的拼接形状（`✅ ||`、`)![`）；
        c. 更一般的：解冲突后与两侧 parent 比行数，只增不减的文档若减了就报。
        ⚠️ b 是**按字符串形状分类**（L-13），只挡得住见过的形状；
        c 最接近「这件事真的发生了吗」，但要处理合并里合法的删除。
        先记着，等第三次发生时形状会更清楚——**不要为了补一道守卫而挑一个
        挡不住下一次的判据**
- [x] 批 M · 外部评审 C/D 部分（飞书可靠性 + 生命周期收敛）—— **独立复核通过
  ...
  - [ ] F 节"当前必需"部分（不含三项 Registry）—— **4 个子批全部完成**，
        ...
        - [x] **U-III** 移除动态 `sys.path` 依赖（教程第 56 章）。
              **126 处 → 70 处**：删掉 `tests/` 里 56 处冗余 + 24 个孤儿 import。
              🔴 发现提示词给的判据**对测试文件不成立**——`sys.path.insert` 是进程级
              副作用而 pytest 把所有模块导进同一进程 ⇒ 删一处再跑全量必然全绿。
              真判据是**单文件独立进程**（48 个改动文件逐个验过）。
              守卫 `tests/test_test_path_bootstrap.py`（6 条）钉住「冗余的不许回来」
  - [ ] 🔴 **薄壳退役** —— U-III 剩下的 **69 处 `sys.path.insert` 卡在这里**，
        它不是"还没顾上删"，是**前提不成立**：
        那 69 处挂的是仓库根的 `skills/`，而全仓还有 **233 处**在用旧名字
        import（`from _contract import ...`）；`skills/` 又被批 U-II 明确排除在
        pip 包之外（三条理由：7 个目录名带连字符做不成包 / 无仓库外消费方 /
        装进去等于占 site-packages 这个共享命名空间的通用名＝R-2 的形状）。
        ⇒ **要删那 69 处，前提是先把 233 处迁到 `easyup_biga.*`** —— 那等于给
        H-I 定的薄壳判死刑，是一个**独立的架构决定**，不该作为"删几个路径操作"
        的副产品夹带进来（F 节的宪章是"打包与依赖清理"，不是"退役薄壳"）。
        **什么条件下才该做**（不是到期日，是判据）：
        ① `skills/` 下的业务脚本不再需要被 OpenClaw 按"技能目录"加载，
           或者它们本身能成为可安装的包（今天 7 个目录名带连字符，做不到）；
        ② 出现第二个需要"在没有仓库 checkout 的环境里跑这些脚本"的真实场景
           —— 今天没有，`bin/biga-card` 拉起的子进程始终在仓库里跑。
        ⚠️ 薄壳本身有既定的退役判据（`deterministic-orchestration.md` §9：
        "读可以宽、写必须严"），这一条是它的**具体落点**，不是新开一条原则
...
- [x] 批 P · 外部评审 E 节（一）：E-16 值一致 + E-18 缺席对号 —— **2026-09-24**。
  ...
  - [ ] **E 节只剩一项**（评审 E 节 = §16–§20 共五项；§17/§18/§19/§20 与 §16 前半已做完）：
    ...
    **口径已定（2026-09-24，裁定 16）** —— 下面是批 1 / 批 2 的输入规格，照着做即可，
    ...
    ⚠️ **六类合计 1719，不是 1721 —— 差的 2 条是探针行，不是第七类**（外部复核指出，
    2026-09-24 已核实）。那 2 条是 `verdict_id=264/265`、`task_id=BIGA-20260302-001`、
    `field=x`、`value=1`、`source=derived:x`，同一秒写入（2026-09-23T07:45:43）——
    测试里那个硬编码 TID，**一次没走 `BIGA_DB_PATH` 的探针直接写进了真库**。
    与仓库记过的那次事故（`probe.sh` 就是为此而生）是同一个形状，这是**第二次**。
    批 1 / 批 2 不必为它们设规则，但**清点时要把它们排除**，否则会去找一条不存在的口径。
    raw 层只追加，这 2 行删不掉。
    ...
    **评审 E 节（§16–§20）至此全部完成。**
    ...
    ✅ **批 5 已完成**（2026-09-24）：`tripped_thresholds` 不再把未知显示成安全
    （无可比字段时不产出该字段 + 进 missing）；契约铁律扩到 `observed`
    （声明了类别就必须说得出出自什么，`parameter` 与历史 `kind=None` 除外）。

    🔴 **裁定 16 五批全部完成，评审 E 节（§16–§20）收官。**
...
- [x] 批 S · 外部评审 A 节剩余四项核实 + Isolation Registry 漂移修复 —— **2026-09-24**。
