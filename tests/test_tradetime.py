...
class TestMarketIsOpenCharacterization:
    ...
    @pytest.mark.parametrize("now,expected,why", [
        ...
        # 🔴 缺陷现状：**三层都不认**的工作日上的法定节假日仍被当成开市。
        #    元旦在兜底表的覆盖区间（2026-09-25 起）**之前** ⇒ 兜底层弃权
        #    ⇒ 退到 weekday ⇒ 仍是这个缺陷。它由日历表负责（刷新覆盖过去 730 天），
        #    而这批特征测试的前提正是「无日历数据」。
        (_dt(2026, 1, 1, 9, 31),   True,  "元旦（周四）—— 兜底区间之外，仍退 weekday"),
        # ✅ 国庆落在兜底表覆盖区间内 ⇒ 无日历数据也判对了（2026-09-25 加的第 3 层）。
        (_dt(2026, 10, 1, 10, 0),  False, "国庆首日（周四）—— 兜底表认识它"),
    ])
