...
_TEST_COUNT_PATTERNS = (
    ...
    # 🔴 第四处，2026-09-25 补。README 正文写的是「**1890 条 Hermetic 测试选中**」——
    #    中间夹了个 `Hermetic`，上面那条 `(\d+)\s*条测试` 就匹配不上了。
    #    后果与徽章那次一模一样：**既不被同步脚本改、也不被本守卫查**，
    #    于是它停在 1890 而同一个文件里的徽章已经是 1891 —— 同一份 README
    #    自己跟自己对不上，而三条正则全绿。
    #    ⚠️ 又一次 L-13：判据按**字符串形状**写，作者多打两个字它就静默失效。
    r"(\d+)\s*条\s*Hermetic\s*测试",
)
