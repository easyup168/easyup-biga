...
🔴 退 2 不代表端点坏了 —— 但**也不要直接断定是限流**。

⚠️ 2026-09-26 实测推翻了本文件原来那句「东财会整体限流：被限流时连已知
可用的端点也 502」。那天的真实情况是 **`push2*` 这一组 host 的上游故障**：
...
⇒ **一次失败什么都证明不了，对照组才让 502 变成信号。**
  分不清「我被限流了」和「那一组挂了」的代价很实在：前者该等，
  后者该走降级链 —— 而本 dataset 2026-09-26 起真的有降级链了。
...
def main() -> int:
    ...
    try:
        ...
        print(f"🔶 没取成：{e}\n")
        print("   这**不**等于端点坏了，但也别直接当成限流。")
        print("   先做对照组：同一时刻试 push2ex / push2his / datacenter-web。")
        print("   它们好而 push2 挂 ⇒ 是那一组的上游故障，等没用，走降级链。")
        print("   全都挂 ⇒ 才可能是限流。别在这个状态下改过滤串。")
        return _v.UNKNOWN
