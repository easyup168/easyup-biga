...
class Result:
    ...
    def render(self) -> str:
        ...
        out.append("")
        out.append("══ 隔离自检全绿 ══" if not self.failed
                   else f"══ {self.failed} 项未通过 ══")
        return "\n".join(out)
...
def _biga_pids() -> list[tuple[int, str]]:
    """找出属于 BigA 的进程。判据是 cwd 或 cmdline 落在 BigA 根下。"""
    out = []
    for d in pathlib.Path("/proc").iterdir():
        ...
        if _under(cwd, BIGA) or str(BIGA) in cmd:
            out.append((pid, cmd[:110] or f"<pid {pid}>"))
    return out
...
def check_i1(res: Result) -> None:
    ...
    res.add(not offenders, f"I-1 · BigA 无写模式打开邻居文件"
            f"（{len(pids)} 个进程 / {checked} 个 fd）",
            "\n".join(offenders) if offenders
            else f"邻居目录 {NEIGHBOUR}" if pids
            else "⚠️ 没找到 BigA 进程 —— 本次检查未覆盖任何东西，不算通过证据")
    if not pids:
        res.rows[-1] = (False, res.rows[-1][1], res.rows[-1][2])


def check_r2(res: Result) -> None:
    """R-2：nvm 的 bin 目录里绝不能有 openclaw。"""
    bad = NVM_BIN / "openclaw"
    res.add(not bad.exists(), "R-2 · nvm bin 里没有 openclaw",
            f"{bad} 存在 —— 邻居的 systemd 会静默改用 BigA 的 binary"
            if bad.exists() else str(NVM_BIN))


def check_ports(res: Result) -> None:
    """端口不重叠。读 /proc/net/tcp，不起任何连接。"""
    ...
    clash = ours & theirs
    res.add(not clash, "端口 · BigA 与邻居不重叠",
            f"BigA 在听 {sorted(ours) or '（无）'}；"
...
def check_i2(res: Result, before: str | None) -> None:
    ...
    if not db.exists():
        res.add(True, "I-2 · 邻居状态库", "邻居未安装，跳过")
        return
    now = str(db.stat().st_mtime_ns)
    if before is None:
        res.add(True, "I-2 · 邻居状态库 mtime（仅记录）",
                f"{now}\n"
                f"⚠️ 没给 --before，这一项不构成证据 —— 邻居自己也在写它。\n"
                f"   正确用法：先记下这个值，做完 BigA 的操作再用 --before 比对")
        return
    res.add(now == before, "I-2 · 邻居状态库在 BigA 操作前后未变",
            f"before={before}\nafter ={now}"
            + ("" if now == before else "\n🔴 变了 —— 要么是我们写的，要么是邻居自己在跑，"
                                        "\n   必须查清楚是哪一种"))
